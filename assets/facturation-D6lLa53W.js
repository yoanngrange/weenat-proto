import"./modulepreload-polyfill-B5Qt9EMX.js";/* empty css             */import{u as $}from"./breadcrumb-CzFWtd6d.js";import{m as y}from"./members-Cr98hVG_.js";import{o as h}from"./orgs-DnFmvGlp.js";import{n as d}from"./network-YzWdS0n_.js";document.addEventListener("DOMContentLoaded",()=>{$(),I()});const c={plan:"Plus",anniversaire:"2022-03-15",numero:"CTR-2022-00418",licencesMembres:y.length,licencesAdherents:h.filter(e=>e.id!==100&&e.statut!=="inactif").length,licencesIntegrations:27,licencesTotal:y.length+h.filter(e=>e.id!==100&&e.statut!=="inactif").length},n={societe:d.nom,siret:d.siret,adresse:d.siege.adresse,cp:d.siege.codePostal,ville:d.siege.ville,pays:d.siege.pays};function I(){const e=document.getElementById("billing-page");if(!e)return;const l=S();e.innerHTML=`
        ...
        <div class="billing-section">
          ...
          <div class="billing-contract-grid">
            ...
            <div class="billing-contract-item">
              ...
              <span class="billing-contract-value">${v(c.anniversaire)}</span>
            </div>
            <div class="billing-contract-item">
            ...
            <form id="billing-addr-form" autocomplete="off">
              <div class="billing-form-grid">
                ...
                <div class="form-row billing-form-full">
                  ...
                  <select id="bf-pays">
                    ${A.map(t=>`<option value="${t.code}"${t.code===n.pays?" selected":""}>${t.label}</option>`).join("")}
                  </select>
                </div>
          ...
          <tbody>
            ${l.map(t=>`
              <tr>
                ...
                <td>${t.dateReglement?v(t.dateReglement):'<span style="color:var(--txt3)">—</span>'}</td>
                <td>${T(t.statut)}</td>
  ...
  `,x()}function x(){var s,u;const e=document.getElementById("bf-pays");e&&(e.addEventListener("change",()=>f(e.value)),f(e.value));const l=document.querySelectorAll(".billing-addr-tab");l.forEach(i=>{i.addEventListener("click",()=>{l.forEach(o=>o.classList.remove("active")),i.classList.add("active"),document.querySelectorAll(".billing-addr-tab-content").forEach(o=>o.style.display="none"),document.getElementById(`tab-content-${i.dataset.tab}`).style.display=""})});const t=document.getElementById("same-delivery-addr"),a=document.getElementById("tab-livraison");t&&a&&t.addEventListener("change",()=>{a.style.display=t.checked?"none":"",t.checked?(l.forEach(i=>i.classList.remove("active")),document.querySelector('[data-tab="facturation"]').classList.add("active"),document.querySelectorAll(".billing-addr-tab-content").forEach(i=>i.style.display="none"),document.getElementById("tab-content-facturation").style.display=""):a.click()}),(s=document.getElementById("billing-addr-save"))==null||s.addEventListener("click",()=>{g("Adresse enregistrée.")}),(u=document.getElementById("billing-addr-cancel"))==null||u.addEventListener("click",()=>{document.getElementById("bf-societe").value=n.societe,document.getElementById("bf-adresse").value=n.adresse,document.getElementById("bf-cp").value=n.cp,document.getElementById("bf-ville").value=n.ville,document.getElementById("bf-pays").value=n.pays,document.getElementById("bf-siret").value=n.siret,document.getElementById("bf-copie-factures").value="",f(n.pays)}),document.querySelectorAll(".billing-pdf-btn").forEach(i=>{i.addEventListener("click",()=>g(`PDF facture ${i.dataset.id} téléchargé.`))}),document.querySelectorAll(".billing-detail-csv-btn").forEach(i=>{i.addEventListener("click",()=>{const o=i.dataset.id,m=`Adhérent;Licences;Options;Montant HT
`,p=new Blob(["\uFEFF"+m],{type:"text/csv;charset=utf-8;"}),r=Object.assign(document.createElement("a"),{href:URL.createObjectURL(p),download:`detail-facture-${o}.csv`});document.body.appendChild(r),r.click(),document.body.removeChild(r),setTimeout(()=>URL.revokeObjectURL(r.href),100)})}),document.querySelectorAll(".billing-detail-btn").forEach(i=>{i.addEventListener("click",()=>g(`Détail facture ${i.dataset.id}.`))}),document.querySelectorAll(".billing-pay-btn").forEach(i=>{i.addEventListener("click",()=>g(`Paiement facture ${i.dataset.id} : redirection…`))})}function f(e){const l=document.getElementById("bf-siret-row"),t=document.getElementById("bf-siret-label"),a=document.getElementById("bf-siret");!l||!t||!a||(e==="FR"?(l.hidden=!1,t.textContent="SIRET",a.placeholder="14 chiffres"):w.includes(e)?(l.hidden=!1,t.textContent="N° TVA intracommunautaire",a.placeholder="FR12345678901"):l.hidden=!0)}function S(){const e=[],l=new Date("2026-04-15"),t=[150,250,100,150,250,150,100,250,150,100];for(let a=11;a>=0;a--){const s=new Date(l.getFullYear(),l.getMonth()-a,1),u=s.getFullYear(),i=String(s.getMonth()+1).padStart(2,"0"),m=a>0?a===2?"en attente":"payée":"en attente",p=m==="payée"?E(new Date(s.getFullYear(),s.getMonth(),18)):null,L=s.getMonth()===11?1980:t[a%t.length];e.push({numero:`FAC-${u}-${i}`,dateEmission:E(new Date(s.getFullYear(),s.getMonth(),1)),montant:L,dateReglement:p,statut:m})}return e.reverse()}function E(e){return`${e.getFullYear()}-${String(e.getMonth()+1).padStart(2,"0")}-${String(e.getDate()).padStart(2,"0")}`}function v(e){if(!e)return"—";const[l,t,a]=e.split("-");return`${a}/${t}/${l}`}function T(e){return e==="payée"?'<span class="statut-badge statut-actif"><i class="bi bi-check-circle-fill"></i> Payée</span>':e==="en attente"?'<span class="statut-badge statut-essai"><i class="bi bi-clock"></i> En attente</span>':e==="échue"?'<span class="statut-badge statut-inactif"><i class="bi bi-exclamation-circle"></i> Échue</span>':`<span class="statut-badge">${e}</span>`}function b(e){return(e||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function g(e){const l=document.createElement("div");l.className="toast",l.textContent=e,document.body.appendChild(l),setTimeout(()=>l.classList.add("show"),10),setTimeout(()=>{l.classList.remove("show"),setTimeout(()=>l.remove(),300)},2500)}const w=["DE","AT","BE","BG","CY","HR","DK","ES","EE","FI","GR","HU","IE","IT","LV","LT","LU","MT","NL","PL","PT","CZ","RO","SK","SI","SE"],A=[{code:"FR",label:"France"},{code:"BE",label:"Belgique"},{code:"CH",label:"Suisse"},{code:"LU",label:"Luxembourg"},{code:"DE",label:"Allemagne"},{code:"ES",label:"Espagne"},{code:"IT",label:"Italie"},{code:"PT",label:"Portugal"},{code:"NL",label:"Pays-Bas"},{code:"AT",label:"Autriche"},{code:"PL",label:"Pologne"},{code:"GB",label:"Royaume-Uni"},{code:"US",label:"États-Unis"},{code:"CA",label:"Canada"},{code:"MA",label:"Maroc"},{code:"TN",label:"Tunisie"},{code:"SN",label:"Sénégal"}];
