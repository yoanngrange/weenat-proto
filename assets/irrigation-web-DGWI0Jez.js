import{I as S,s as A,g as X}from"./irrigations-C22Dtsoo.js";import{b as J,d as K}from"./store-BGemiHCf.js";import{p as N}from"./plots-DHZTuoR0.js";import{I as Z}from"./constants-D_Yu58_o.js";const j=new Date().toISOString().split("T")[0],ii=1;J(N);const M=N.filter(i=>i.orgId===ii);function C(i){const t=["jan.","fév.","mar.","avr.","mai","jun.","jul.","aoû.","sep.","oct.","nov.","déc."],[,e,l]=i.split("-");return`${parseInt(l)} ${t[parseInt(e)-1]} ${i.split("-")[0]}`}function z(i){const t=["jan","fév","mar","avr","mai","jun","jul","aoû","sep","oct","nov","déc"],[,e,l]=i.split("-");return`${parseInt(l)} ${t[parseInt(e)-1]}`}function V(i,t){return i?'<div class="iw-chk iw-chk--on"><i class="bi bi-check"></i></div>':'<div class="iw-chk"></div>'}function ti(i){const t=i.crop,e=i.irrigation||null,l=i.substrate?"Substrat : "+i.substrate:i.texture||null,s=[t,e,l].filter(Boolean);return s.length?`<span class="iw-plot-info">${s.join(" · ")}</span>`:`<span class="iw-plot-info iw-plot-info--miss">type d'irrigation non renseigné</span>`}function F(){return[...M].sort((i,t)=>i.name.localeCompare(t.name,"fr"))}function ei(i){if(i.length<2)return"";const t=[...i].sort((g,k)=>g.iso<k.iso?-1:1),e=["jan","fév","mar","avr","mai","jun","jul","aoû","sep","oct","nov","déc"],l=new Date(t[0].iso),s=new Date(t[t.length-1].iso),a=Math.max(s-l,1),o=600,n=26,c=5,m=5,u=n-m-4,y=u-m-3,$=n+c+13,I=$+2,L=m+2,r=g=>Math.round(L+(g-l)/a*(o-2*L)),v=g=>{const[,k,q]=g.split("-");return`${parseInt(q)}/${parseInt(k)}`},w=[],f=new Date(l.getFullYear(),l.getMonth()+1,1);for(;f<=s;)w.push(new Date(f)),f.setMonth(f.getMonth()+1);const p=w.map(g=>`<line x1="${r(g)}" y1="${n-6}" x2="${r(g)}" y2="${n+c+6}" stroke="#C0BEB8" stroke-width="1"/>`).join(""),d=[l,...w,s],b=d.slice(0,-1).map((g,k)=>{const q=r(g),h=r(d[k+1]);return h-q<16?"":`<text x="${Math.round((q+h)/2)}" y="${y}" font-size="9" fill="#B0AEA8" text-anchor="middle">${e[g.getMonth()]}</text>`}).join(""),E=g=>{const k=["jan","fév","mar","avr","mai","jun","jul","aoû","sep","oct","nov","déc"],[,q,h]=g.split("-");return`${parseInt(h)} ${k[parseInt(q)-1]}`},x=t.map(g=>{const k=g.real?"#FF8500":"#FFB705",q=r(new Date(g.iso)),h=`${E(g.iso)} · ${g.mm} mm`;return`<circle cx="${q}" cy="${u}" r="${m}" fill="${k}" stroke="#fff" stroke-width="1.5" pointer-events="none"/>
    <circle cx="${q}" cy="${u}" r="${m+6}" fill="transparent" pointer-events="all" data-tip="${h}"/>`}).join("");return`<div class="iw-timeline">
    <svg width="${o}" height="${I}" viewBox="0 0 ${o} ${I}" style="width:100%;display:block">
      ${b}
  ...
  </div>`}function si(i){const t=i.filter(a=>a.real),e=i.filter(a=>!a.real),l=t.reduce((a,o)=>a+o.mm,0),s=e.reduce((a,o)=>a+o.mm,0);return`<div class="iw-cumuls-bar">
  ...
  </div>`}function Y({title:i,params:t,plotNames:e,addedCount:l,filterPlotId:s}){const{ov:a,close:o}=T(),n=t.map($=>`
    ...
    </div>`,a.querySelector("#iw-conf-edit").addEventListener("click",()=>{S.splice(S.length-l,l),A(),o()}),a.querySelector("#iw-conf-voir").addEventListener("click",()=>{o(),U(s??null)}),a.querySelector("#iw-conf-close").addEventListener("click",o)}function ni(i){const t=document.createElement("div");t.className="iw-toast",t.textContent=i,document.body.appendChild(t),requestAnimationFrame(()=>t.classList.add("iw-toast--in")),setTimeout(()=>{t.classList.remove("iw-toast--in"),setTimeout(()=>t.remove(),400)},3e3)}function G(i,t){const e=M.filter(o=>i.has(o.id)&&(!o.irrigation||o.irrigation==="Non irrigué"));if(!e.length){t();return}const{ov:l,close:s}=T();l.innerHTML=`
      ...
      <div class="iw-modal-body">
        ...
        <div class="iw-field">
          ...
          <select class="iw-input" id="iw-ask-type">
            ...
            ${Z.map(o=>`<option value="${o}">${o}</option>`).join("")}
          </select>
        </div>
      </div>
    ...
    <div id="iw-sel-list">${H(i)}</div>
  </div>`}function R(i){const t=i.size;if(t===0)return'<div class="iw-sel-empty">Aucune parcelle sélectionnée</div>';const e=[...i].map(l=>{var s;return(s=M.find(a=>a.id===l))==null?void 0:s.name}).filter(Boolean);return`<div class="iw-sel-summary">
    <div class="iw-sel-title">${t===1?e[0]:`${t} parcelles`}</div>
    ${e.map(l=>`<div class="iw-sel-name">· ${l}</div>`).join("")}
  </div>`}function P(i,t,e){var l;i.querySelectorAll(".iw-plot-row").forEach(s=>{s.addEventListener("click",()=>{const a=+s.dataset.pid;t.has(a)?t.delete(a):t.add(a);const o=i.querySelector("#iw-sel-list");o&&(o.innerHTML=H(t)),P(i,t,e),e()})}),(l=i.querySelector("#iw-sel-all"))==null||l.addEventListener("click",()=>{const s=F(),a=s.some(n=>!t.has(n.id));s.forEach(n=>a?t.add(n.id):t.delete(n.id));const o=i.querySelector("#iw-sel-list");o&&(o.innerHTML=H(t)),i.querySelector("#iw-sel-all").textContent=a?"Tout désélectionner":"Tout sélectionner",P(i,t,e),e()})}function O(i=null){const t=new Set((i==null?void 0:i.ids)??[]);let e=j,l=10;const{ov:s,close:a}=T();s.innerHTML=`
    <div class="iw-modal iw-modal--wide">
    ...
    </div>`,s.querySelector("#iw-close").addEventListener("click",a),s.querySelector("#iw-cancel").addEventListener("click",a),s.querySelector("#iw-date").addEventListener("change",n=>{e=n.target.value}),s.querySelector("#iw-qty").addEventListener("input",n=>{l=parseInt(n.target.value)||1});function o(){s.querySelector("#iw-save").disabled=!i&&t.size===0;const n=s.querySelector("#iw-sel-sum");n&&(n.innerHTML=R(t))}i||P(s,t,o),s.querySelector("#iw-save").addEventListener("click",()=>{const n=i?new Set(i.ids):t;G(n,()=>{const c=e>j;n.forEach(u=>{S.push({iso:e,mm:l,real:!c,plotId:u,fromStrategy:!1})});const m=[...n].map(u=>{var y;return(y=M.find($=>$.id===u))==null?void 0:y.name}).filter(Boolean);A(),a(),Y({title:n.size>1?"Irrigations enregistrées":"Irrigation enregistrée",params:[{label:"Date",value:C(e)},{label:"Quantité",value:`${l} mm`}],plotNames:m,addedCount:n.size,filterPlotId:n.size===1?[...n][0]:null})})})}function B(i=null){const t=new Set((i==null?void 0:i.ids)??[]);let e=j,l=new Date(new Date().setMonth(new Date().getMonth()+4)).toISOString().split("T")[0],s=10,a=7,o="form";const{ov:n,close:c}=T();function m(){if(!e||!l||a<=0)return[];const r=[],v=new Date(l);let w=new Date(e);for(;w<=v&&r.length<100;)r.push(new Date(w)),w.setDate(w.getDate()+a);return r}function u(){const r=m(),v=r.length>0?`<div class="iw-preview-ok">↗ ${r.length} irrigations · ${r.length*s} mm au total</div>`:'<div class="iw-preview-empty">Ajustez les dates et la fréquence</div>';return`
      <div class="iw-two-col">
        <div class="iw-panel iw-panel--params">
        ...
        ${Q(t,i)}
      </div>`}function y(){const r=m(),v=r.filter(h=>h.toISOString().slice(0,10)<j),w=r.filter(h=>h.toISOString().slice(0,10)>=j),f=i?i.ids.length:t.size;function p(h){const D=h.toISOString().slice(0,10),W=D>=j?"#FFB705":"#FF8500";return`<div class="iw-occ-row">
        <span>${C(D)}</span>
        <span style="font-weight:700;color:${W}">${s} mm</span>
      </div>`}const d=i?M.filter(h=>i.ids.includes(h.id)):M.filter(h=>t.has(h.id)),b=4,E=d.slice(0,b),x=d.slice(b),g=`
      ...
      </div>`,k=d.reduce((h,D)=>h+(D.area??0),0),q=k>0?Math.round(r.length*s*k*10):0;return`
      <div class="iw-apercu-summary">
        <div>
          <div class="iw-apercu-big">${r.length} irrigations</div>
          <div class="iw-apercu-sub">${z(e)} → ${z(l)} · tous les ${a}j</div>
        </div>
        <div style="text-align:right">
          <div class="iw-apercu-big">${r.length*s} mm</div>
          <div class="iw-apercu-sub">${f} parcelle${f>1?"s":""} · ${s} mm/irrig.${q>0?` · ${q.toLocaleString("fr-FR")} m³`:""}</div>
        </div>
      </div>
      ${g}
    ...
    </div>`,n.querySelector("#iw-close").addEventListener("click",c),n.querySelector("#iw-cancel").addEventListener("click",c),n.querySelector("#iw-back-btn").addEventListener("click",()=>{o="form",L()}),n.querySelector("#iw-next-btn").addEventListener("click",()=>{o="apercu",L()}),n.querySelector("#iw-conf-btn").addEventListener("click",()=>{const r=m(),v=i?new Set(i.ids):t;G(v,()=>{var p,d;const w=X();r.forEach(b=>{const E=b.toISOString().slice(0,10);v.forEach(x=>{S.push({iso:E,mm:s,real:E<=j,plotId:x,fromStrategy:!0,seasonId:w})})});const f=[...v].map(b=>{var E;return(E=M.find(x=>x.id===b))==null?void 0:E.name}).filter(Boolean);A(),c(),Y({title:"Saison enregistrée",params:[{label:"Début",value:C(((p=r[0])==null?void 0:p.toISOString().slice(0,10))??"")},{label:"Fin",value:C(((d=r[r.length-1])==null?void 0:d.toISOString().slice(0,10))??"")},{label:"Quantité",value:`${s} mm/irrig.`},{label:"Fréquence",value:`tous les ${a} jours`},{label:"Irrigations générées",value:`${r.length*v.size}`}],plotNames:f,addedCount:r.length*v.size,filterPlotId:v.size===1?[...v][0]:null})})}),L()}function li(i,t){let e=i.iso,l=i.mm;const{ov:s,close:a}=T();s.innerHTML=`
    <div class="iw-modal" style="max-width:420px;width:95%">
    ...
    </div>`,s.querySelector("#iw-close").addEventListener("click",a),s.querySelector("#iw-cancel").addEventListener("click",a),s.querySelector("#iw-edit-date").addEventListener("change",o=>{e=o.target.value}),s.querySelector("#iw-edit-qty").addEventListener("input",o=>{const n=parseInt(o.target.value);n>0&&(l=n)}),s.querySelector("#iw-delete").addEventListener("click",()=>{const o=S.indexOf(i);o>-1&&S.splice(o,1),A(),a(),t()}),s.querySelector("#iw-save").addEventListener("click",()=>{i.iso=e,i.mm=l,i.real=e<=j,A(),a(),t()})}function _(i,t){let e=!1,l=!1;const{ov:s,close:a}=T();function o(){return new Set(S.filter(c=>c.plotId===i&&c.fromStrategy&&c.seasonId).map(c=>c.seasonId))}function n(){var y,$,I,L,r,v,w;const c=s.querySelector(".iw-modal-body");c.innerHTML=`
      ...
      </div>`:""}`;const m=M.find(f=>f.id===i),u=m?{ids:[m.id],label:m.name}:null;(y=c.querySelector("#strat-modify"))==null||y.addEventListener("click",()=>{a(),B(u)}),($=c.querySelector("#strat-stop"))==null||$.addEventListener("click",()=>{e=!0,l=!1,n()}),(I=c.querySelector("#strat-delete"))==null||I.addEventListener("click",()=>{l=!0,e=!1,n()}),(L=c.querySelector("#stop-cancel"))==null||L.addEventListener("click",()=>{e=!1,n()}),(r=c.querySelector("#stop-confirm"))==null||r.addEventListener("click",()=>{const f=o();S.splice(0,S.length,...S.filter(p=>!(p.plotId===i&&f.has(p.seasonId)&&!p.real))),A(),a(),t()}),(v=c.querySelector("#del-cancel"))==null||v.addEventListener("click",()=>{l=!1,n()}),(w=c.querySelector("#del-confirm"))==null||w.addEventListener("click",()=>{const f=o();S.splice(0,S.length,...S.filter(p=>!(p.plotId===i&&f.has(p.seasonId)))),A(),a(),t()})}s.innerHTML=`
    ...
    </div>`,s.querySelector("#iw-close").addEventListener("click",a),s.querySelector("#iw-cancel").addEventListener("click",a),n()}function ai(){var c,m;const i=new Set(M.map(u=>u.id)),t=[...S].sort((u,y)=>u.iso<y.iso?-1:1);let e=((c=t[0])==null?void 0:c.iso)??j,l=((m=t[t.length-1])==null?void 0:m.iso)??j;const{ov:s,close:a}=T();function o(){return F().map(u=>{const y=i.has(u.id);return`<div class="iw-plot-row${y?" iw-plot-row--sel":""}" data-pid="${u.id}">
        ${V(y)}
    ...
    </div>`,s.querySelector("#iw-close").addEventListener("click",a),s.querySelector("#iw-cancel").addEventListener("click",a),s.querySelector("#iw-exp-start").addEventListener("change",u=>{e=u.target.value}),s.querySelector("#iw-exp-end").addEventListener("change",u=>{l=u.target.value});function n(){s.querySelectorAll("#iw-export-plots .iw-plot-row").forEach(u=>{u.addEventListener("click",()=>{const y=+u.dataset.pid;i.has(y)?i.delete(y):i.add(y),s.querySelector("#iw-export-plots").innerHTML=o(),n()})})}n(),s.querySelector("#iw-do-export").addEventListener("click",()=>{const u=F().filter(p=>i.has(p.id));if(!u.length){ni("Sélectionnez au moins une parcelle");return}const y=new Set;for(const p of S)p.iso>=e&&p.iso<=l&&y.add(p.iso);const $=[...y].sort(),I=["Date",...u.map(p=>p.name)].join(";"),L=$.map(p=>{const d=u.map(b=>{const E=S.filter(x=>x.iso===p&&x.plotId===b.id).reduce((x,g)=>x+g.mm,0);return E>0?E:""});return[p,...d].join(";")}),r=[I,...L].join(`
`),v=new Blob([r],{type:"text/csv;charset=utf-8;"}),w=URL.createObjectURL(v),f=document.createElement("a");f.href=w,f.download=`irrigations_${e}_${l}.csv`,document.body.appendChild(f),f.click(),setTimeout(()=>{URL.revokeObjectURL(w),f.remove()},100),a()})}function U(i=null){let t=i?String(i):"all";const{ov:e,close:l}=T();function s(){return t==="all"?S:S.filter(c=>c.plotId===+t)}function a(){if(t==="all")return null;const c=M.find(m=>String(m.id)===t);return c?{ids:[c.id],label:c.name}:null}const o=F().map(c=>`<option value="${c.id}"${String(c.id)===t?" selected":""}>${c.name}</option>`).join("");e.innerHTML=`
    <div class="iw-modal iw-modal--full">
      ...
      <div class="iw-full-body" id="iw-full-body"></div>
        ...
        </div>`,c.querySelector("#iw-add-saisie").addEventListener("click",()=>{l(),O(a())}),c.querySelector("#iw-add-saison").addEventListener("click",()=>{l(),B(a())});return}const $={};c.querySelectorAll("details[data-section]").forEach(d=>{$[d.dataset.section]=d.open});const I=[...m].filter(d=>!d.real).sort((d,b)=>d.iso<b.iso?-1:1),L=[...m].filter(d=>d.real).sort((d,b)=>d.iso>b.iso?-1:1);function r(d){var q;const b=S.indexOf(d),E=d.real?"#FF8500":"#FFB705",[,x,g]=d.iso.split("-"),k=((q=M.find(h=>h.id===d.plotId))==null?void 0:q.name)??"—";return`<div class="iw-list-item">
        ...
        <div style="flex:1">
          <div class="iw-list-label">${k}</div>
          <div class="iw-list-status" style="color:${E}">${d.real?"Effectuée":"Planifiée"}</div>
        </div>
      ...
      ${I.length?`
        <details class="iw-section-details" data-section="planifiees" open>
          <summary class="iw-section-summary">Planifiées <span class="iw-section-count">${I.length}</span></summary>
          <div class="iw-section-body">${I.map(r).join("")}</div>
        </details>`:""}
    `,c.querySelectorAll("details[data-section]").forEach(d=>{const b=d.dataset.section;b in $&&(d.open=$[b])}),c.querySelectorAll(".iw-edit-btn").forEach(d=>{d.addEventListener("click",()=>{const b=S[+d.dataset.iidx];b&&li(b,n)})}),c.querySelectorAll(".iw-del-btn").forEach(d=>{d.addEventListener("click",()=>{const b=+d.dataset.iidx;b>=0&&(S.splice(b,1),A(),n())})}),(w=c.querySelector("#iw-add-saisie"))==null||w.addEventListener("click",()=>{l(),O(a())}),(f=c.querySelector("#iw-add-saison"))==null||f.addEventListener("click",()=>{l(),B(a())}),(p=c.querySelector("#iw-strat-inline"))==null||p.addEventListener("click",()=>_(+t,n))}n()}(()=>{const i=Object.assign(document.createElement("div"),{id:"irr-data-tip",className:"irr-data-tip"});document.body.appendChild(i),document.addEventListener("mouseover",t=>{const e=t.target.closest("[data-tip]");if(!e){i.style.opacity="0";return}i.textContent=e.dataset.tip,i.style.opacity="1"}),document.addEventListener("mousemove",t=>{i.style.left=t.clientX+14+"px",i.style.top=t.clientY-36+"px"}),document.addEventListener("mouseout",t=>{var e;(e=t.relatedTarget)!=null&&e.closest("[data-tip]")||(i.style.opacity="0")})})();window.WebIrrig={openSaisie:O,openSaison:B,openVoirIrrigations:U};
