import{b as F,I as E,s as M}from"./irrigations-MeXOpmvP.js";import{d as X,e as J}from"./store-DRCbE3kB.js";import{p as G}from"./plots-urzhAbT6.js";const A=new Date().toISOString().split("T")[0],K=1;X(G);const h=G.filter(i=>i.orgId===K);function C(i){const s=["jan.","fév.","mar.","avr.","mai","jun.","jul.","aoû.","sep.","oct.","nov.","déc."],[,t,a]=i.split("-");return`${parseInt(a)} ${s[parseInt(t)-1]} ${i.split("-")[0]}`}function R(i){const s=["jan","fév","mar","avr","mai","jun","jul","aoû","sep","oct","nov","déc"],[,t,a]=i.split("-");return`${parseInt(a)} ${s[parseInt(t)-1]}`}function P(i,s){return i?'<div class="iw-chk iw-chk--on"><i class="bi bi-check"></i></div>':s?'<div class="iw-chk iw-chk--partial">−</div>':'<div class="iw-chk"></div>'}function Z(i){const s=i.crop,t=i.irrigation||null,a=i.substrate?"Substrat : "+i.substrate:i.texture||null,l=[s,t,a].filter(Boolean);return l.length?`<span class="iw-plot-info">${l.join(" · ")}</span>`:`<span class="iw-plot-info iw-plot-info--miss">type d'irrigation non renseigné</span>`}function N(i,s){const t=[],a=new Set(i);for(const l of s){const e=l.ids.filter(o=>i.has(o));e.length&&(e.length===l.ids.length?(t.push(l.label),l.ids.forEach(o=>a.delete(o))):e.forEach(o=>{const r=h.find(c=>c.id===o);r&&(t.push(r.name),a.delete(o))}))}for(const l of a){const e=h.find(o=>o.id===l);e&&t.push(e.name)}return t}function ii(i,s){const t=i.size;if(t===1){const l=h.find(e=>e.id===[...i][0]);return{text:(l==null?void 0:l.name)??"Parcelle",count:null}}const a=s.find(l=>l.ids.length===t&&l.ids.every(e=>i.has(e)));return a?{text:a.label,count:a.ids.length}:{text:`${t} parcelles`,count:null}}function ti(i,s){const t=new Set(i),a=[];for(const l of s)if(l.ids.length>=2&&l.ids.every(e=>i.has(e))){const e=l.ids.map(o=>{var r;return(r=h.find(c=>c.id===o))==null?void 0:r.name}).filter(Boolean);a.push({title:`Groupe "${l.label}"`,names:e}),l.ids.forEach(o=>t.delete(o))}if(t.size>0){const l=[...t].map(e=>{var o;return(o=h.find(r=>r.id===e))==null?void 0:o.name}).filter(Boolean);a.push({title:a.length>0?"Autres parcelles":null,names:l})}return a}function ei(i){if(i.length<2)return"";const s=[...i].sort((d,b)=>d.iso<b.iso?-1:1),t=["jan","fév","mar","avr","mai","jun","jul","aoû","sep","oct","nov","déc"],a=new Date(s[0].iso),l=new Date(s[s.length-1].iso),e=Math.max(l-a,1),o=600,r=26,c=5,p=5,w=r-p-4,n=w-p-3,u=r+c+13,m=u+2,g=d=>Math.round((d-a)/e*o),q=d=>{const[,b,x]=d.split("-");return`${parseInt(x)}/${parseInt(b)}`},v=[],y=new Date(a.getFullYear(),a.getMonth()+1,1);for(;y<=l;)v.push(new Date(y)),y.setMonth(y.getMonth()+1);const f=v.map(d=>`<line x1="${g(d)}" y1="${r-6}" x2="${g(d)}" y2="${r+c+6}" stroke="#C0BEB8" stroke-width="1"/>`).join(""),k=[a,...v,l],$=k.slice(0,-1).map((d,b)=>{const x=g(d),L=g(k[b+1]);return L-x<16?"":`<text x="${Math.round((x+L)/2)}" y="${n}" font-size="9" fill="#B0AEA8" text-anchor="middle">${t[d.getMonth()]}</text>`}).join(""),S=d=>{const b=["jan","fév","mar","avr","mai","jun","jul","aoû","sep","oct","nov","déc"],[,x,L]=d.split("-");return`${parseInt(L)} ${b[parseInt(x)-1]}`},j=s.map(d=>{const b=d.real?"#FF8500":"#FFB705",x=g(new Date(d.iso)),L=`${S(d.iso)} · ${d.mm} mm`;return`<circle cx="${x}" cy="${w}" r="${p}" fill="${b}" stroke="#fff" stroke-width="1.5" pointer-events="none"/>
    ...
    <svg width="${o}" height="${m}" viewBox="0 0 ${o} ${m}" style="width:100%;display:block">
      ${$}
      <rect x="0" y="${r}" width="${o}" height="${c}" rx="${c/2}" fill="#E0DED8"/>
      ${f}
      ${j}
  ...
  </div>`}function si(i){const s=i.filter(e=>e.real),t=i.filter(e=>!e.real),a=s.reduce((e,o)=>e+o.mm,0),l=t.reduce((e,o)=>e+o.mm,0);return`<div class="iw-cumuls-bar">
  ...
  </div>`}function V({title:i,params:s,labels:t,addedCount:a,filterLabel:l}){const{ov:e,close:o}=T(),r=s.map(u=>`
    ...
    <div class="iw-modal" style="max-width:400px;width:95%">
      ...
      <div class="iw-modal-body">
        ...
        <div class="iw-field">
          ...
          <select class="iw-input" id="iw-ask-type">
            ...
            ${li.map(o=>`<option value="${o}">${o}</option>`).join("")}
          </select>
        </div>
      </div>
      ...
      ${P(o,!1)}
      <div><div class="iw-plot-name">${e.name}</div>${Z(e)}</div>
  ...
  </div>`}function z(i,s){if(i.size===0)return'<div class="iw-sel-empty">Aucune parcelle sélectionnée</div>';const a=ii(i,s),l=ti(i,s);return`<div class="iw-sel-summary">
    ...
    `).join("")}
  </div>`}function B(i,s,t,a){i.querySelectorAll(".iw-group-card").forEach(l=>{l.addEventListener("click",()=>{const e=l.dataset.gids.split(",").map(Number),o=e.every(c=>t.has(c));e.forEach(c=>o?t.delete(c):t.add(c));const r=i.querySelector("#iw-sel-list");r&&(r.innerHTML=O(s,t)),B(i,s,t,a),a()})}),i.querySelectorAll(".iw-plot-row").forEach(l=>{l.addEventListener("click",()=>{const e=+l.dataset.pid;t.has(e)?t.delete(e):t.add(e);const o=i.querySelector("#iw-sel-list");o&&(o.innerHTML=O(s,t)),B(i,s,t,a),a()})})}function H(i=null){const s=F(h),t=new Set((i==null?void 0:i.ids)??[]);let a=A,l=10;const{ov:e,close:o}=T();e.innerHTML=`
    <div class="iw-modal iw-modal--wide">
    ...
    </div>`,e.querySelector("#iw-close").addEventListener("click",o),e.querySelector("#iw-cancel").addEventListener("click",o),e.querySelector("#iw-date").addEventListener("change",c=>{a=c.target.value}),e.querySelector("#iw-qty").addEventListener("input",c=>{l=parseInt(c.target.value)||1});function r(){e.querySelector("#iw-save").disabled=!i&&t.size===0;const c=e.querySelector("#iw-sel-sum");c&&(c.innerHTML=z(t,s))}i||B(e,s,t,r),e.querySelector("#iw-save").addEventListener("click",()=>{const c=i?new Set(i.ids):t;Y(c,()=>{const p=a>A,w=i?[i.label]:N(c,s);w.forEach(n=>{E.push({iso:a,mm:l,real:!p,label:n,fromStrategy:!1})}),M(),o(),V({title:w.length>1?"Irrigations enregistrées":"Irrigation enregistrée",params:[{label:"Date",value:C(a)},{label:"Quantité",value:`${l} mm`}],labels:w,addedCount:w.length,filterLabel:w.length===1?w[0]:null})})})}function I(i=null){const s=F(h),t=new Set((i==null?void 0:i.ids)??[]);let a=A,l=new Date(new Date().setMonth(new Date().getMonth()+4)).toISOString().split("T")[0],e=10,o=7,r="form";const{ov:c,close:p}=T();function w(){if(!a||!l||o<=0)return[];const v=[],y=new Date(l);let f=new Date(a);for(;f<=y&&v.length<100;)v.push(new Date(f)),f.setDate(f.getDate()+o);return v}function n(){const v=w(),y=v.length>0?`<div class="iw-preview-ok">↗ ${v.length} irrigations · ${v.length*e} mm au total</div>`:'<div class="iw-preview-empty">Ajustez les dates et la fréquence</div>';return`
      <div class="iw-two-col">
        <div class="iw-panel iw-panel--params">
      ...
      </div>`}const S=i?h.filter(L=>i.ids.includes(L.id)):h.filter(L=>t.has(L.id)),j=4,d=S.slice(0,j),b=S.slice(j),x=`
      <div class="iw-apercu-plots">
        <div class="iw-section-lbl">Parcelles concernées</div>
        ${d.map(L=>`<span class="iw-apercu-plot-tag">${L.name}</span>`).join("")}${b.length?`<span class="iw-apercu-plot-more">+ ${b.length} autre${b.length>1?"s":""}</span>`:""}
      </div>`;return`
    ...
    `}function m(){const v=i?i.ids.length:t.size,y=w(),f=c.querySelector("#iw-next-btn");f&&(f.disabled=v===0||y.length===0)}function g(){var y,f,k,$;i||B(c,s,t,m);const v=()=>{const S=w(),j=c.querySelector(".iw-preview-ok, .iw-preview-empty");j&&(j.className=S.length>0?"iw-preview-ok":"iw-preview-empty",j.textContent=S.length>0?`↗ ${S.length} irrigations · ${S.length*e} mm au total`:"Ajustez les dates et la fréquence"),m()};(y=c.querySelector("#iw-debut"))==null||y.addEventListener("change",S=>{a=S.target.value,v()}),(f=c.querySelector("#iw-fin"))==null||f.addEventListener("change",S=>{l=S.target.value,v()}),(k=c.querySelector("#iw-qty"))==null||k.addEventListener("input",S=>{e=parseInt(S.target.value)||1,v()}),($=c.querySelector("#iw-freq"))==null||$.addEventListener("input",S=>{o=parseInt(S.target.value)||1,v()})}function q(){var y;const v=c.querySelector(".iw-modal");v.querySelector(".iw-modal-body").innerHTML=r==="form"?n():u(),v.querySelector("#iw-back-btn").style.display=r==="form"?"none":"",v.querySelector("#iw-next-btn").style.display=r==="form"?"":"none",v.querySelector("#iw-conf-btn").style.display=r==="form"?"none":"",r==="form"?(m(),g()):(y=c.querySelector("#iw-apercu-more"))==null||y.addEventListener("click",f=>{c.querySelector("#iw-apercu-rest").style.display="",f.target.remove()})}c.innerHTML=`
    <div class="iw-modal iw-modal--wide">
      <div class="iw-modal-hd">
    ...
    </div>`,c.querySelector("#iw-close").addEventListener("click",p),c.querySelector("#iw-cancel").addEventListener("click",p),c.querySelector("#iw-back-btn").addEventListener("click",()=>{r="form",q()}),c.querySelector("#iw-next-btn").addEventListener("click",()=>{r="apercu",q()}),c.querySelector("#iw-conf-btn").addEventListener("click",()=>{const v=w(),y=i?new Set(i.ids):t;Y(y,()=>{var k,$;const f=i?[i.label]:N(y,s);v.forEach(S=>{const j=S.toISOString().slice(0,10);f.forEach(d=>{E.push({iso:j,mm:e,real:j<=A,label:d,fromStrategy:!0})})}),M(),p(),V({title:"Saison enregistrée",params:[{label:"Début",value:C(((k=v[0])==null?void 0:k.toISOString().slice(0,10))??"")},{label:"Fin",value:C((($=v[v.length-1])==null?void 0:$.toISOString().slice(0,10))??"")},{label:"Quantité",value:`${e} mm/irrig.`},{label:"Fréquence",value:`tous les ${o} jours`},{label:"Irrigations générées",value:`${v.length*f.length}`}],labels:f,addedCount:v.length*f.length,filterLabel:f.length===1?f[0]:null})})}),q()}function ai(i,s){var p,w;const a=F(h).find(n=>n.label===i.label);let l=i.iso,e=i.mm,o="single";const{ov:r,close:c}=T();r.innerHTML=`
    <div class="iw-modal" style="max-width:420px;width:95%">
    ...
    </div>`,r.querySelector("#iw-close").addEventListener("click",c),r.querySelector("#iw-cancel").addEventListener("click",c),r.querySelector("#iw-edit-date").addEventListener("change",n=>{l=n.target.value}),r.querySelector("#iw-edit-qty").addEventListener("input",n=>{const u=parseInt(n.target.value);u>0&&(e=u)}),a&&((p=r.querySelector("#scope-single"))==null||p.addEventListener("click",n=>{o="single",r.querySelectorAll(".iw-scope-btn").forEach(u=>u.classList.remove("iw-scope-btn--on")),n.target.classList.add("iw-scope-btn--on")}),(w=r.querySelector("#scope-group"))==null||w.addEventListener("click",n=>{o="group",r.querySelectorAll(".iw-scope-btn").forEach(u=>u.classList.remove("iw-scope-btn--on")),n.target.classList.add("iw-scope-btn--on")})),r.querySelector("#iw-delete").addEventListener("click",()=>{const n=E.indexOf(i);n>-1&&E.splice(n,1),M(),c(),s()}),r.querySelector("#iw-save").addEventListener("click",()=>{i.iso=l,i.mm=e,i.real=l<=A,o==="group"&&a&&E.filter(n=>n.label===i.label).forEach(n=>{n.mm=e}),M(),c(),s()})}function _(i,s){let t=!1,a=!1;const{ov:l,close:e}=T();function o(c){const p=F(h).find(n=>n.label===c);if(p)return{ids:p.ids,label:p.label};const w=h.find(n=>n.name===c);return w?{ids:[w.id],label:w.name}:null}function r(){var p,w,n,u,m,g,q;const c=l.querySelector(".iw-modal-body");c.innerHTML=`
      ...
      </div>`:""}`,(p=c.querySelector("#strat-modify"))==null||p.addEventListener("click",()=>{e(),I(o(i))}),(w=c.querySelector("#strat-stop"))==null||w.addEventListener("click",()=>{t=!0,a=!1,r()}),(n=c.querySelector("#strat-delete"))==null||n.addEventListener("click",()=>{a=!0,t=!1,r()}),(u=c.querySelector("#stop-cancel"))==null||u.addEventListener("click",()=>{t=!1,r()}),(m=c.querySelector("#stop-confirm"))==null||m.addEventListener("click",()=>{E.splice(0,E.length,...E.filter(v=>!(v.label===i&&!v.real))),M(),e(),s()}),(g=c.querySelector("#del-cancel"))==null||g.addEventListener("click",()=>{a=!1,r()}),(q=c.querySelector("#del-confirm"))==null||q.addEventListener("click",()=>{E.splice(0,E.length,...E.filter(v=>v.label!==i)),M(),e(),s()})}l.innerHTML=`
    ...
    </div>`,l.querySelector("#iw-close").addEventListener("click",e),l.querySelector("#iw-cancel").addEventListener("click",e),r()}function oi(){var p,w;const i=F(h),s=new Set(h.map(n=>n.id)),t=[...E].sort((n,u)=>n.iso<u.iso?-1:1);let a=((p=t[0])==null?void 0:p.iso)??A,l=((w=t[t.length-1])==null?void 0:w.iso)??A;const{ov:e,close:o}=T();function r(){return h.map(n=>{const u=s.has(n.id);return`<div class="iw-plot-row${u?" iw-plot-row--sel":""}" data-pid="${n.id}">
        ${P(u,!1)}
...
`),y=new Blob([v],{type:"text/csv;charset=utf-8;"}),f=URL.createObjectURL(y),k=document.createElement("a");k.href=f,k.download=`irrigations_${a}_${l}.csv`,document.body.appendChild(k),k.click(),setTimeout(()=>{URL.revokeObjectURL(f),k.remove()},100),o()})}function U(i=null){const s=F(h);let t=i?i.ids.length>1?i.label:String(i.ids[0]):"all";const{ov:a,close:l}=T();function e(){if(!t||t==="all")return E;const n=s.find(m=>m.label===t);if(n)return E.filter(m=>m.label===n.label);const u=h.find(m=>String(m.id)===String(t));if(u){const m=new Set(s.filter(g=>g.ids.includes(u.id)).map(g=>g.label));return E.filter(g=>g.label===u.name||m.has(g.label))}return E}function o(n){if(!n||n==="all")return null;const u=s.find(g=>g.label===n);if(u)return{ids:u.ids,label:u.label};const m=h.find(g=>String(g.id)===String(n));return m?{ids:[m.id],label:m.name}:null}const r=h.filter(n=>!s.some(u=>u.ids.includes(n.id))),c=s.map(n=>{const u=n.ids.map(m=>h.find(g=>g.id===m)).filter(Boolean).map(m=>`<option value="${m.id}"${String(m.id)===String(t)?" selected":""}>-${m.name}</option>`).join("");return`<optgroup label="${n.label} (${n.ids.length} parcelles)">
    ...
    <div class="iw-modal iw-modal--full">
      ...
      <div class="iw-full-body" id="iw-full-body"></div>
    </div>`,a.querySelector("#iw-close").addEventListener("click",l),a.querySelector("#iw-filter").addEventListener("change",n=>{t=n.target.value,w()}),a.querySelector("#iw-export-btn").addEventListener("click",oi);function w(){var $,S,j;const n=a.querySelector("#iw-full-body"),u=e(),m=u.some(d=>d.fromStrategy),g=a.querySelector("#iw-strat-btn");if(g.style.display=m&&t!=="all"?"":"none",g.onclick=()=>_(t,w),!u.length){n.innerHTML=`
        <div class="iw-empty">
        ...
        </div>`,n.querySelector("#iw-add-saisie").addEventListener("click",()=>{l(),H(o(t))}),n.querySelector("#iw-add-saison").addEventListener("click",()=>{l(),I(o(t))});return}const q={};n.querySelectorAll("details[data-section]").forEach(d=>{q[d.dataset.section]=d.open});const v=[...u].filter(d=>!d.real).sort((d,b)=>d.iso<b.iso?-1:1),y=[...u].filter(d=>d.real).sort((d,b)=>d.iso>b.iso?-1:1);function f(d){const b=E.indexOf(d),x=d.real?"#FF8500":"#FFB705",[,L,D]=d.iso.split("-");return`<div class="iw-list-item">
        ...
        <div style="flex:1">
          <div class="iw-list-label">${d.label}</div>
          <div class="iw-list-status" style="color:${x}">${d.real?"Réalisée":"Planifiée"}</div>
        </div>
        <div class="iw-list-mm" style="color:${x}">${d.mm} mm</div>
    ...
    `,n.querySelectorAll("details[data-section]").forEach(d=>{const b=d.dataset.section;b in q&&(d.open=q[b])}),n.querySelectorAll(".iw-edit-btn").forEach(d=>{d.addEventListener("click",()=>{const b=E[+d.dataset.iidx];b&&ai(b,w)})}),n.querySelectorAll(".iw-del-btn").forEach(d=>{d.addEventListener("click",()=>{const b=+d.dataset.iidx;b>=0&&(E.splice(b,1),M(),w())})}),($=n.querySelector("#iw-add-saisie"))==null||$.addEventListener("click",()=>{l(),H(o(t))}),(S=n.querySelector("#iw-add-saison"))==null||S.addEventListener("click",()=>{l(),I(o(t))}),(j=n.querySelector("#iw-strat-inline"))==null||j.addEventListener("click",()=>_(t,w))}w()}(()=>{const i=Object.assign(document.createElement("div"),{id:"irr-data-tip",className:"irr-data-tip"});document.body.appendChild(i),document.addEventListener("mouseover",s=>{const t=s.target.closest("[data-tip]");if(!t){i.style.opacity="0";return}i.textContent=t.dataset.tip,i.style.opacity="1"}),document.addEventListener("mousemove",s=>{i.style.left=s.clientX+14+"px",i.style.top=s.clientY-36+"px"}),document.addEventListener("mouseout",s=>{var t;(t=s.relatedTarget)!=null&&t.closest("[data-tip]")||(i.style.opacity="0")})})();window.WebIrrig={openSaisie:H,openSaison:I,openVoirIrrigations:U};
