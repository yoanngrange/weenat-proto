import{b as F,I as E,s as M}from"./irrigations-cQAUTeq7.js";import{p as U}from"./plots-BEYIGwLd.js";const A=new Date().toISOString().split("T")[0],W=1,h=U.filter(i=>i.orgId===W);function B(i){const s=["jan.","fév.","mar.","avr.","mai","jun.","jul.","aoû.","sep.","oct.","nov.","déc."],[,t,a]=i.split("-");return`${parseInt(a)} ${s[parseInt(t)-1]} ${i.split("-")[0]}`}function R(i){const s=["jan","fév","mar","avr","mai","jun","jul","aoû","sep","oct","nov","déc"],[,t,a]=i.split("-");return`${parseInt(a)} ${s[parseInt(t)-1]}`}function C(i,s){return i?'<div class="iw-chk iw-chk--on"><i class="bi bi-check"></i></div>':s?'<div class="iw-chk iw-chk--partial">−</div>':'<div class="iw-chk"></div>'}function X(i){const s=i.crop,t=i.irrigation&&i.irrigation!=="Pas d'irrigation"?i.irrigation:null;return s&&t?`<span class="iw-plot-info">${s} · ${t}</span>`:s?`<span class="iw-plot-info">${s}</span>`:t?`<span class="iw-plot-info">${t}</span>`:`<span class="iw-plot-info iw-plot-info--miss">type d'irrigation non renseigné</span>`}function V(i,s){const t=[],a=new Set(i);for(const l of s){const e=l.ids.filter(o=>i.has(o));e.length&&(e.length===l.ids.length?(t.push(l.label),l.ids.forEach(o=>a.delete(o))):e.forEach(o=>{const d=h.find(c=>c.id===o);d&&(t.push(d.name),a.delete(o))}))}for(const l of a){const e=h.find(o=>o.id===l);e&&t.push(e.name)}return t}function J(i,s){const t=i.size;if(t===1){const l=h.find(e=>e.id===[...i][0]);return{text:(l==null?void 0:l.name)??"Parcelle",count:null}}const a=s.find(l=>l.ids.length===t&&l.ids.every(e=>i.has(e)));return a?{text:a.label,count:a.ids.length}:{text:`${t} parcelles`,count:null}}function K(i,s){const t=new Set(i),a=[];for(const l of s)if(l.ids.length>=2&&l.ids.every(e=>i.has(e))){const e=l.ids.map(o=>{var d;return(d=h.find(c=>c.id===o))==null?void 0:d.name}).filter(Boolean);a.push({title:`Groupe "${l.label}"`,names:e}),l.ids.forEach(o=>t.delete(o))}if(t.size>0){const l=[...t].map(e=>{var o;return(o=h.find(d=>d.id===e))==null?void 0:o.name}).filter(Boolean);a.push({title:a.length>0?"Autres parcelles":null,names:l})}return a}function Z(i){if(i.length<2)return"";const s=[...i].sort((r,b)=>r.iso<b.iso?-1:1),t=["jan","fév","mar","avr","mai","jun","jul","aoû","sep","oct","nov","déc"],a=new Date(s[0].iso),l=new Date(s[s.length-1].iso),e=Math.max(l-a,1),o=600,d=26,c=5,p=5,w=d-p-4,n=w-p-3,u=d+c+13,m=u+2,g=r=>Math.round((r-a)/e*o),q=r=>{const[,b,x]=r.split("-");return`${parseInt(x)}/${parseInt(b)}`},v=[],y=new Date(a.getFullYear(),a.getMonth()+1,1);for(;y<=l;)v.push(new Date(y)),y.setMonth(y.getMonth()+1);const f=v.map(r=>`<line x1="${g(r)}" y1="${d-6}" x2="${g(r)}" y2="${d+c+6}" stroke="#C0BEB8" stroke-width="1"/>`).join(""),k=[a,...v,l],$=k.slice(0,-1).map((r,b)=>{const x=g(r),L=g(k[b+1]);return L-x<16?"":`<text x="${Math.round((x+L)/2)}" y="${n}" font-size="9" fill="#B0AEA8" text-anchor="middle">${t[r.getMonth()]}</text>`}).join(""),S=r=>{const b=["jan","fév","mar","avr","mai","jun","jul","aoû","sep","oct","nov","déc"],[,x,L]=r.split("-");return`${parseInt(L)} ${b[parseInt(x)-1]}`},j=s.map(r=>{const b=r.real?"#FF8500":"#FFB705",x=g(new Date(r.iso)),L=`${S(r.iso)} · ${r.mm} mm`;return`<circle cx="${x}" cy="${w}" r="${p}" fill="${b}" stroke="#fff" stroke-width="1.5" pointer-events="none"/>
  ...
  </div>`}function ii(i){const s=i.filter(e=>e.real),t=i.filter(e=>!e.real),a=s.reduce((e,o)=>e+o.mm,0),l=t.reduce((e,o)=>e+o.mm,0);return`<div class="iw-cumuls-bar">
  ...
  </div>`}function N({title:i,params:s,labels:t,addedCount:a,filterLabel:l}){const{ov:e,close:o}=D(),d=s.map(u=>`
    ...
    <div class="iw-modal iw-modal--confirm">
    ...
    </div>`,e.querySelector("#iw-conf-edit").addEventListener("click",()=>{E.splice(E.length-a,a),M(),o()}),e.querySelector("#iw-conf-voir").addEventListener("click",()=>{o();const u=l?(()=>{const m=F(h).find(q=>q.label===l);if(m)return{ids:m.ids,label:m.label};const g=h.find(q=>q.name===l);return g?{ids:[g.id],label:g.name}:null})():null;Y(u)}),e.querySelector("#iw-conf-close").addEventListener("click",o)}function ti(i){const s=document.createElement("div");s.className="iw-toast",s.textContent=i,document.body.appendChild(s),requestAnimationFrame(()=>s.classList.add("iw-toast--in")),setTimeout(()=>{s.classList.remove("iw-toast--in"),setTimeout(()=>s.remove(),400)},3e3)}function D(){const i=document.createElement("div");i.className="iw-overlay",document.body.appendChild(i),requestAnimationFrame(()=>i.classList.add("iw-overlay--in"));const s=()=>{i.classList.remove("iw-overlay--in"),setTimeout(()=>i.remove(),250)};return i.addEventListener("click",t=>{t.target===i&&s()}),{ov:i,close:s}}function P(i,s){const t=h.filter(e=>!i.some(o=>o.ids.includes(e.id))),a=i.map(e=>{const o=e.ids.every(p=>s.has(p)),d=e.ids.some(p=>s.has(p)),c=e.ids.map(p=>h.find(w=>w.id===p)).filter(Boolean).map(p=>{const w=s.has(p.id);return`<div class="iw-plot-row iw-plot-row--child${w?" iw-plot-row--sel":""}" data-pid="${p.id}">
          ${C(w,!1)}
      ...
      ${C(o,!1)}
      <div><div class="iw-plot-name">${e.name}</div>${X(e)}</div>
    </div>`}).join("");return a+(t.length?`${i.length?'<div class="iw-section-lbl" style="margin-top:10px">Autres parcelles</div>':""}${l}`:"")}function G(i,s,t){if(t){const a=h.filter(l=>t.ids.includes(l.id));return`<div class="iw-panel iw-panel--fixed">
    ...
    <div id="iw-sel-list">${P(i,s)}</div>
  </div>`}function z(i,s){if(i.size===0)return'<div class="iw-sel-empty">Aucune parcelle sélectionnée</div>';const a=J(i,s),l=K(i,s);return`<div class="iw-sel-summary">
    ...
    `).join("")}
  </div>`}function O(i,s,t,a){i.querySelectorAll(".iw-group-card").forEach(l=>{l.addEventListener("click",()=>{const e=l.dataset.gids.split(",").map(Number),o=e.every(c=>t.has(c));e.forEach(c=>o?t.delete(c):t.add(c));const d=i.querySelector("#iw-sel-list");d&&(d.innerHTML=P(s,t)),O(i,s,t,a),a()})}),i.querySelectorAll(".iw-plot-row").forEach(l=>{l.addEventListener("click",()=>{const e=+l.dataset.pid;t.has(e)?t.delete(e):t.add(e);const o=i.querySelector("#iw-sel-list");o&&(o.innerHTML=P(s,t)),O(i,s,t,a),a()})})}function I(i=null){const s=F(h),t=new Set((i==null?void 0:i.ids)??[]);let a=A,l=10;const{ov:e,close:o}=D();e.innerHTML=`
    ...
    </div>`,e.querySelector("#iw-close").addEventListener("click",o),e.querySelector("#iw-cancel").addEventListener("click",o),e.querySelector("#iw-date").addEventListener("change",c=>{a=c.target.value}),e.querySelector("#iw-qty").addEventListener("input",c=>{l=parseInt(c.target.value)||1});function d(){e.querySelector("#iw-save").disabled=!i&&t.size===0;const c=e.querySelector("#iw-sel-sum");c&&(c.innerHTML=z(t,s))}i||O(e,s,t,d),e.querySelector("#iw-save").addEventListener("click",()=>{const c=i?new Set(i.ids):t,p=a>A,w=i?[i.label]:V(c,s);w.forEach(n=>{E.push({iso:a,mm:l,real:!p,label:n,fromStrategy:!1})}),M(),o(),N({title:w.length>1?"Irrigations enregistrées":"Irrigation enregistrée",params:[{label:"Date",value:B(a)},{label:"Quantité",value:`${l} mm`}],labels:w,addedCount:w.length,filterLabel:w.length===1?w[0]:null})})}function H(i=null){const s=F(h),t=new Set((i==null?void 0:i.ids)??[]);let a=A,l=new Date(new Date().setMonth(new Date().getMonth()+4)).toISOString().split("T")[0],e=10,o=7,d="form";const{ov:c,close:p}=D();function w(){if(!a||!l||o<=0)return[];const v=[],y=new Date(l);let f=new Date(a);for(;f<=y&&v.length<100;)v.push(new Date(f)),f.setDate(f.getDate()+o);return v}function n(){const v=w(),y=v.length>0?`<div class="iw-preview-ok">↗ ${v.length} irrigations · ${v.length*e} mm au total</div>`:'<div class="iw-preview-empty">Ajustez les dates et la fréquence</div>';return`
      <div class="iw-two-col">
        <div class="iw-panel iw-panel--params">
        ...
        ${G(s,t,i)}
      </div>`}function u(){const v=w(),y=v.filter(L=>L.toISOString().slice(0,10)<A),f=v.filter(L=>L.toISOString().slice(0,10)>=A),k=i?i.ids.length:t.size;function $(L){const T=L.toISOString().slice(0,10),Q=T>=A?"#FFB705":"#FF8500";return`<div class="iw-occ-row">
        <span>${B(T)}</span>
        <span style="font-weight:700;color:${Q}">${e} mm</span>
    ...
    </div>`,c.querySelector("#iw-close").addEventListener("click",p),c.querySelector("#iw-cancel").addEventListener("click",p),c.querySelector("#iw-back-btn").addEventListener("click",()=>{d="form",q()}),c.querySelector("#iw-next-btn").addEventListener("click",()=>{d="apercu",q()}),c.querySelector("#iw-conf-btn").addEventListener("click",()=>{var k,$;const v=w(),y=i?new Set(i.ids):t,f=i?[i.label]:V(y,s);v.forEach(S=>{const j=S.toISOString().slice(0,10);f.forEach(r=>{E.push({iso:j,mm:e,real:j<=A,label:r,fromStrategy:!0})})}),M(),p(),N({title:"Saison enregistrée",params:[{label:"Début",value:B(((k=v[0])==null?void 0:k.toISOString().slice(0,10))??"")},{label:"Fin",value:B((($=v[v.length-1])==null?void 0:$.toISOString().slice(0,10))??"")},{label:"Quantité",value:`${e} mm/irrig.`},{label:"Fréquence",value:`tous les ${o} jours`},{label:"Irrigations générées",value:`${v.length*f.length}`}],labels:f,addedCount:v.length*f.length,filterLabel:f.length===1?f[0]:null})}),q()}function ei(i,s){var p,w;const a=F(h).find(n=>n.label===i.label);let l=i.iso,e=i.mm,o="single";const{ov:d,close:c}=D();d.innerHTML=`
    ...
    </div>`,l.querySelector("#iw-close").addEventListener("click",e),l.querySelector("#iw-cancel").addEventListener("click",e),d()}function si(){var p,w;const i=F(h),s=new Set(h.map(n=>n.id)),t=[...E].sort((n,u)=>n.iso<u.iso?-1:1);let a=((p=t[0])==null?void 0:p.iso)??A,l=((w=t[t.length-1])==null?void 0:w.iso)??A;const{ov:e,close:o}=D();function d(){return h.map(n=>{const u=s.has(n.id);return`<div class="iw-plot-row${u?" iw-plot-row--sel":""}" data-pid="${n.id}">
        ${C(u,!1)}
...
`),y=new Blob([v],{type:"text/csv;charset=utf-8;"}),f=URL.createObjectURL(y),k=document.createElement("a");k.href=f,k.download=`irrigations_${a}_${l}.csv`,document.body.appendChild(k),k.click(),setTimeout(()=>{URL.revokeObjectURL(f),k.remove()},100),o()})}function Y(i=null){const s=F(h);let t=i?i.ids.length>1?i.label:String(i.ids[0]):"all";const{ov:a,close:l}=D();function e(){if(!t||t==="all")return E;const n=s.find(m=>m.label===t);if(n)return E.filter(m=>m.label===n.label);const u=h.find(m=>String(m.id)===String(t));if(u){const m=new Set(s.filter(g=>g.ids.includes(u.id)).map(g=>g.label));return E.filter(g=>g.label===u.name||m.has(g.label))}return E}function o(n){if(!n||n==="all")return null;const u=s.find(g=>g.label===n);if(u)return{ids:u.ids,label:u.label};const m=h.find(g=>String(g.id)===String(n));return m?{ids:[m.id],label:m.name}:null}const d=h.filter(n=>!s.some(u=>u.ids.includes(n.id))),c=s.map(n=>{const u=n.ids.map(m=>h.find(g=>g.id===m)).filter(Boolean).map(m=>`<option value="${m.id}"${String(m.id)===String(t)?" selected":""}>-${m.name}</option>`).join("");return`<optgroup label="${n.label} (${n.ids.length} parcelles)">
    ...
    <div class="iw-modal iw-modal--full">
      ...
      <div class="iw-full-body" id="iw-full-body"></div>
    </div>`,a.querySelector("#iw-close").addEventListener("click",l),a.querySelector("#iw-filter").addEventListener("change",n=>{t=n.target.value,w()}),a.querySelector("#iw-export-btn").addEventListener("click",si);function w(){var $,S,j;const n=a.querySelector("#iw-full-body"),u=e(),m=u.some(r=>r.fromStrategy),g=a.querySelector("#iw-strat-btn");if(g.style.display=m&&t!=="all"?"":"none",g.onclick=()=>_(t,w),!u.length){n.innerHTML=`
        <div class="iw-empty">
        ...
        </div>`,n.querySelector("#iw-add-saisie").addEventListener("click",()=>{l(),I(o(t))}),n.querySelector("#iw-add-saison").addEventListener("click",()=>{l(),H(o(t))});return}const q={};n.querySelectorAll("details[data-section]").forEach(r=>{q[r.dataset.section]=r.open});const v=[...u].filter(r=>!r.real).sort((r,b)=>r.iso<b.iso?-1:1),y=[...u].filter(r=>r.real).sort((r,b)=>r.iso>b.iso?-1:1);function f(r){const b=E.indexOf(r),x=r.real?"#FF8500":"#FFB705",[,L,T]=r.iso.split("-");return`<div class="iw-list-item">
        <div class="iw-list-stripe" style="background:${x}"></div>
        <div class="iw-list-date"><span style="font-size:15px;font-weight:700;color:${x}">${T}</span><span style="font-size:10px;color:${x}">/${L}</span></div>
        <div style="flex:1">
    ...
    `,n.querySelectorAll("details[data-section]").forEach(r=>{const b=r.dataset.section;b in q&&(r.open=q[b])}),n.querySelectorAll(".iw-edit-btn").forEach(r=>{r.addEventListener("click",()=>{const b=E[+r.dataset.iidx];b&&ei(b,w)})}),n.querySelectorAll(".iw-del-btn").forEach(r=>{r.addEventListener("click",()=>{const b=+r.dataset.iidx;b>=0&&(E.splice(b,1),M(),w())})}),($=n.querySelector("#iw-add-saisie"))==null||$.addEventListener("click",()=>{l(),I(o(t))}),(S=n.querySelector("#iw-add-saison"))==null||S.addEventListener("click",()=>{l(),H(o(t))}),(j=n.querySelector("#iw-strat-inline"))==null||j.addEventListener("click",()=>_(t,w))}w()}(()=>{const i=Object.assign(document.createElement("div"),{id:"irr-data-tip",className:"irr-data-tip"});document.body.appendChild(i),document.addEventListener("mouseover",s=>{const t=s.target.closest("[data-tip]");if(!t){i.style.opacity="0";return}i.textContent=t.dataset.tip,i.style.opacity="1"}),document.addEventListener("mousemove",s=>{i.style.left=s.clientX+14+"px",i.style.top=s.clientY-36+"px"}),document.addEventListener("mouseout",s=>{var t;(t=s.relatedTarget)!=null&&t.closest("[data-tip]")||(i.style.opacity="0")})})();window.WebIrrig={openSaisie:I,openSaison:H,openVoirIrrigations:Y};
