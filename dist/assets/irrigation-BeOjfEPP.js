import{p as P,a as z,c as J}from"./mobile-BGxJ_Ibv.js";import{b as O,I as L,s as C}from"./irrigations-Dbn0IxRs.js";import{e as ce}from"./store-DZP_FQdH.js";import"./modulepreload-polyfill-B5Qt9EMX.js";import"./plots-hjmThJP8.js";import"./sensors-DDe0aa9W.js";import"./orgs-Cx4wNzLL.js";import"./network-YzWdS0n_.js";import"./members-C_ssoUx1.js";function R(a){return a.style.display="flex",a.style.flexDirection="column",a.style.overflowY="hidden",a}const I=new Date().toISOString().split("T")[0];function H(a){const h=["jan.","fév.","mar.","avr.","mai","jun.","jul.","aoû.","sep.","oct.","nov.","déc."],[s,l,p]=a.split("-");return`${parseInt(p)} ${h[parseInt(l)-1]} ${s}`}function K(a,h,s,l){if(s){const t=s.ids.map(n=>{var y;return(y=h.find($=>$.id===n))==null?void 0:y.name}).filter(Boolean);return[{title:`Groupe "${s.label}"`,names:t}]}const p=new Set(a),b=[];for(const t of l)if(t.ids.length>=2&&t.ids.every(n=>a.has(n))){const n=t.ids.map(y=>{var $;return($=h.find(x=>x.id===y))==null?void 0:$.name}).filter(Boolean);b.push({title:`Groupe "${t.label}"`,names:n}),t.ids.forEach(y=>p.delete(y))}if(p.size>0){const t=[...p].map(n=>{var y;return(y=h.find($=>$.id===n))==null?void 0:y.name}).filter(Boolean);b.push({title:b.length>0?"Autres parcelles":null,names:t})}return b}function Z(a){const h=["jan","fév","mar","avr","mai","jun","jul","aoû","sep","oct","nov","déc"],[,s,l]=a.split("-");return`${parseInt(l)} ${h[parseInt(s)-1]}`}function Y(a){return!a||a===I?"":a<I?'<span class="irr-pill irr-pill--past">Passée · Réalisée</span>':'<span class="irr-pill irr-pill--future">Future · Planifiée</span>'}function V(a,h){return a?'<div class="irr-chk irr-chk--on"></div>':h?'<div class="irr-chk irr-chk--partial">−</div>':'<div class="irr-chk"></div>'}function ee(a,h,s){const l=[],p=new Set(a);for(const b of h){const t=b.ids.filter(n=>a.has(n));t.length&&(t.length===b.ids.length?(l.push(b.label),b.ids.forEach(n=>p.delete(n))):t.forEach(n=>{const y=s.find($=>$.id===n);y&&(l.push(y.name),p.delete(n))}))}for(const b of p){const t=s.find(n=>n.id===b);t&&l.push(t.name)}return l}function ie(a,h,s){const l=h.filter(t=>!a.some(n=>n.ids.includes(t.id))),p=a.map(t=>{const n=t.ids.every(x=>s.has(x)),y=t.ids.some(x=>s.has(x)),$=t.ids.map(x=>h.find(E=>E.id===x)).filter(Boolean).map(x=>{const E=s.has(x.id);return`<div class="irr-plot-row irr-plot-row--child${E?" irr-plot-row--sel":""}" data-pid="${x.id}">
          <div class="irr-plot-name">${x.name}</div>
          ${V(E,!1)}
        </div>`}).join("");return`<div class="irr-group-card${n?" irr-group-card--sel":""}" data-gids="${t.ids.join(",")}">
      ...
      ${V(n,y)}
    </div>${$}`}).join(""),b=l.map(t=>{const n=s.has(t.id);return`<div class="irr-plot-row${n?" irr-plot-row--sel":""}" data-pid="${t.id}">
      <div><div class="irr-plot-name">${t.name}</div>${ue(t)}</div>
      ${V(n,!1)}
    </div>`}).join("");return p+(l.length?`${a.length?'<div class="irr-section-lbl" style="margin-top:10px">Autres parcelles</div>':""}${b}`:"")}function Q({title:a,body:h,onSave:s,onDelete:l,saveLabel:p="Enregistrer"}){var y,$;const b=document.getElementById("phone-screen");(y=document.querySelector(".irr-sheet-overlay"))==null||y.remove();const t=document.createElement("div");t.className="irr-sheet-overlay",t.innerHTML=`
    <div class="irr-sheet">
      <div class="irr-sheet-handle"></div>
      <div class="irr-sheet-title">${a}</div>
    ...
    </div>`,t.querySelector(".irr-sheet-body").appendChild(typeof h=="string"?Object.assign(document.createElement("div"),{innerHTML:h}):h),b.appendChild(t),requestAnimationFrame(()=>t.classList.add("irr-sheet-overlay--open"));const n=()=>{t.classList.remove("irr-sheet-overlay--open"),setTimeout(()=>t.remove(),280)};return t.addEventListener("click",x=>{x.target===t&&n()}),($=t.querySelector("#iss-del"))==null||$.addEventListener("click",()=>{l==null||l(),n()}),t.querySelector("#iss-save").addEventListener("click",()=>{s==null||s(),n()}),t}const de=["Pivot","Enrouleur","Rampe","Goutte à goutte","Goutte à goutte enterré","Micro aspersion","Couverture intégrale","Gravitaire","Aspersion"];function te(a,h,s){const l=h.filter(n=>a.has(n.id)&&(!n.irrigation||n.irrigation==="Non irrigué"));if(!l.length){s();return}const p=document.createElement("div");p.innerHTML=`
    ...
    <select id="irr-type-ask" style="width:100%;padding:12px;border-radius:10px;border:1px solid #E0DED8;font-size:15px;font-family:inherit;background:#fff;color:#1c1c1e">
      ...
      ${de.map(n=>`<option value="${n}">${n}</option>`).join("")}
    </select>`;const t=Q({title:"Type d'irrigation",body:p,saveLabel:"Confirmer",onDelete:()=>s(),onSave:()=>{const n=p.querySelector("#irr-type-ask").value;n&&l.forEach(y=>{y.irrigation=n,ce(y.id,{irrigation:n})}),s()}}).querySelector("#iss-del");t&&(t.textContent="Passer",t.style.cssText+=";background:#F5F4F0;border-color:#E0DED8;color:#636366")}function ue(a){const h=a.crop,s=a.irrigation||null,l=a.texture||null,p=[h,s||"<em>type irrigation non renseigné</em>",l].filter(Boolean);return!h&&!s?'<span class="irr-plot-info irr-plot-info--miss">Non renseigné</span>':`<span class="irr-plot-info">${p.join(" · ")}</span>`}function ve(a,h,s=null){let l=new Set((s==null?void 0:s.ids)??[]),p=I,b=10;const t=O(a);function n(){return ie(t,a,l)}function y(m){m.querySelector("#irr-date-hint").innerHTML=Y(p)}function $(m){const u=l.size,e=m.querySelector(".irr-summary"),i=m.querySelector(".irr-save-btn");!e||!i||(s?(e.textContent="",i.disabled=!1,i.textContent="Enregistrer"):(e.textContent=u===0?"":`${u} parcelle${u>1?"s":""} · ${b} mm`,i.disabled=u===0,i.textContent="Enregistrer"))}function x(m){m.querySelectorAll(".irr-group-card").forEach(u=>{u.addEventListener("click",()=>{const e=u.dataset.gids.split(",").map(Number),i=e.every(r=>l.has(r));e.forEach(r=>i?l.delete(r):l.add(r)),m.querySelector("#irr-sel-list").innerHTML=n(),x(m),$(m)})}),m.querySelectorAll(".irr-plot-row").forEach(u=>{u.addEventListener("click",()=>{const e=+u.dataset.pid;l.has(e)?l.delete(e):l.add(e),m.querySelector("#irr-sel-list").innerHTML=n(),x(m),$(m)})})}const E=s?a.filter(m=>s.ids.includes(m.id)):[],A=s?`<div class="irr-presel-banner">
       ...
       ${E.map(m=>`<div style="font-size:13px;color:#636366;padding:2px 4px">· ${m.name}</div>`).join("")}`:`<div id="irr-sel-list">${n()}</div>`,q=R(P(`
  ...
  `));q.dataset.dirty="true",s||x(q),$(q),q.querySelector("#irr-back").addEventListener("click",z),q.querySelector("#irr-date").addEventListener("change",m=>{p=m.target.value,y(q),$(q)}),q.querySelector("#irr-qty").addEventListener("input",m=>{b=parseInt(m.target.value)||1,$(q)}),q.querySelector(".irr-save-btn").addEventListener("click",()=>{const m=s?new Set(s.ids):l;te(m,a,()=>{const u=p>I,e=s?[s.label]:ee(m,t,a);e.forEach(d=>{L.push({iso:p,mm:b,real:!u,label:d,fromStrategy:!1})}),C();const i=e.length===1?e[0]:"all",r=K(m,a,s,t),o=r.reduce((d,v)=>d+v.names.length,0);J(),re({title:o>1?"Irrigations enregistrées":"Irrigation enregistrée",params:[{label:"Date",value:H(p)},{label:"Quantité",value:`${b} mm`}],parcelSections:r,isFut:u,plots:a,calFilter:i,addedCount:e.length,stackDepth:1})})})}function U(a,h,s=null,l=null){let p=new Set((s==null?void 0:s.ids)??[]),b=I,t=new Date(new Date().setMonth(new Date().getMonth()+4)).toISOString().split("T")[0],n=10,y=7;const $=O(a);function x(){if(!b||!t||y<=0)return[];const u=[],e=new Date(t);let i=new Date(b);for(;i<=e&&u.length<100;)u.push(new Date(i)),i.setDate(i.getDate()+y);return u}function E(){const u=x();p.size;const e=u.length>0?`<span class="irr-preview-ok">↗ ${u.length} irrigations · ${u.length*n} mm au total</span>`:'<span style="color:#8e8e93">Ajustez les dates et la fréquence</span>';return`
      ...
      <div class="irr-fields-row">
        <div class="irr-field-card">
          <div class="irr-field-lbl">Quantité <span class="irr-unit">(mm)</span></div>
          <input type="number" class="irr-qty-input" id="strat-qty" value="${n}" min="1" />
          <div class="irr-field-hint">par parcelle</div>
        </div>
        <div class="irr-field-card">
  ...
  `));m.dataset.dirty="true",q(m),A(m),m.querySelector("#strat-back").addEventListener("click",z),m.querySelector(".irr-save-btn").addEventListener("click",()=>{const u=x();pe(m,a,p,b,t,n,y,u,h,s,l)})}function pe(a,h,s,l,p,b,t,n,y,$,x=null){var v;const E=s.size,A=5;n.length-A;function q(c){const w=c.toISOString().slice(0,10)>=I?"#FFB705":"#FF8500",g=c.toISOString().slice(0,10);return`<div class="irr-occ-row">
      <span style="flex:1;font-size:13px;color:#1c1c1e">${H(g)}</span>
      <span style="font-size:13px;font-weight:700;color:${w}">${b} mm</span>
    </div>`}const m=n.filter(c=>c.toISOString().slice(0,10)<I),u=n.filter(c=>c.toISOString().slice(0,10)>=I),e=u.slice(0,A),i=u.slice(A),r=m.length?`
    ...
    <div class="irr-scroll-body">
      <div class="irr-apercu-summary">
        <div>
          <div class="irr-apercu-big">${n.length} irrigations</div>
          <div class="irr-apercu-sub">${Z(l)} → ${Z(p)} · tous les ${t}j</div>
        </div>
        <div style="text-align:right">
          <div class="irr-apercu-big">${n.length*b} mm</div>
  ...
  `));d.dataset.dirty="true",d.querySelector("#apercu-back").addEventListener("click",z),(v=d.querySelector("#apercu-show-more"))==null||v.addEventListener("click",c=>{d.querySelector("#apercu-rest").style.display="",c.target.remove()}),d.querySelector("#apercu-confirm").addEventListener("click",()=>{const c=O(h),S=$?new Set($.ids):s;te(S,h,()=>{const w=$?[$.label]:ee(S,c,h),g=w.length===1?w[0]:"all",k=K(S,h,$,c);x&&L.splice(0,L.length,...L.filter(f=>!(w.includes(f.label)&&f.fromStrategy&&!f.real))),n.forEach(f=>{const F=f.toISOString().slice(0,10);w.forEach(j=>{L.push({iso:F,mm:b,real:F<=I,label:j,fromStrategy:!0})})}),C(),J(),re({title:"Saison enregistrée",params:[{label:"Début",value:H(l)},{label:"Fin",value:H(p)},{label:"Quantité",value:`${b} mm/irrig.`},{label:"Fréquence",value:`tous les ${t} jours`},{label:"Irrigations générées",value:`${n.length*w.length}`}],parcelSections:k,isFut:!0,plots:h,calFilter:g,addedCount:n.length*w.length,stackDepth:2})})})}function re({title:a,params:h,parcelSections:s=[],isFut:l,plots:p,calFilter:b,addedCount:t=0,stackDepth:n=3}){const y=(h||[]).map(e=>`
    ...
    <div class="irr-detail-header">
      <div class="irr-header-row">
        <div style="width:60px"></div>
        <span class="irr-header-title">${a.includes("aison")?"Saison":a.includes("rrigations")?"Enregistrées":"Enregistrée"}</span>
        <div style="width:60px"></div>
      </div>
    </div>
    <div class="irr-scroll-body irr-confirm-body">
      ...
      <div class="irr-confirm-title">${a}</div>
  ...
  `));u.querySelector("#conf-edit").addEventListener("click",()=>{t>0&&L.splice(L.length-t,t),z()}),u.querySelector("#conf-back").addEventListener("click",()=>{for(let e=0;e<=n;e++)z()}),u.querySelector("#conf-calendar").addEventListener("click",()=>{me(p,b)})}const fe=["jan","fév","mar","avr","mai","jun","jul","aoû","sep","oct","nov","déc"];function me(a,h){const s=O(a);let l=h||"";function p(e,i,r){if(!e)return null;const o=i.find(v=>v.label===e);if(o)return{ids:o.ids,label:o.label};const d=r.find(v=>String(v.id)===String(e))??r.find(v=>v.name===e);return d?{ids:[d.id],label:d.name}:null}function b(){if(!l||l==="all")return L;const e=s.find(r=>r.label===l);if(e)return L.filter(r=>r.label===e.label);const i=a.find(r=>String(r.id)===String(l));if(i){const r=new Set(s.filter(o=>o.ids.includes(i.id)).map(o=>o.label));return L.filter(o=>o.label===i.name||r.has(o.label))}return L}function t(e){const i=e.filter(v=>v.real),r=e.filter(v=>!v.real),o=i.reduce((v,c)=>v+c.mm,0),d=r.reduce((v,c)=>v+c.mm,0);return`<div class="irr-cumuls-bar">
    ...
    </div>`}function y(e,i,r){var g,k;const d=O(a).find(f=>f.label===e.label),v=document.createElement("div");let c="single",S=e.iso,w=e.mm;v.innerHTML=`
