...
<text transform="translate(${$e},${N}) rotate(-90)" font-size="10" font-family="-apple-system,sans-serif" fill="#4CAF50" font-weight="600">${H.label}</text>`}).join("");let L="",D="";for(let H=o;H<o+n;H++){const Y=new Date(d.getTime()+H*864e5);if(Y.getDate()===1){const W=b(H).toFixed(1);L+=`<line x1="${W}" y1="${l.t}" x2="${W}" y2="${S}" stroke="rgba(0,0,0,.18)" stroke-width="1" stroke-dasharray="3,4"/>`;const Q=new Date(Y.getFullYear(),Y.getMonth()+1,1),$e=Math.round((Q-d)/864e5),N=Math.min($e,o+n-1),G=b((H+N)/2).toFixed(1),U=String(Y.getFullYear()).slice(-2),O=`${_s[Y.getMonth()]} ${U}`;D+=`<text x="${G}" y="${i-6}" text-anchor="middle" font-size="12" font-family="-apple-system,sans-serif" fill="#8e8e93">${O}</text>`}}const T=[0,p,r].map(H=>{const Y=(x(H)+4).toFixed(1);return`<text x="${l.l-6}" y="${Y}" text-anchor="end" font-size="12" font-family="-apple-system,sans-serif" fill="#8e8e93">${H}</text>`}).join("")+`<text x="${l.l-6}" y="${l.t-7}" text-anchor="end" font-size="11" font-family="-apple-system,sans-serif" fill="#8e8e93">mm</text>`,V=7,te=a.slice(o,o+n).map((H,Y)=>({x:b(o+Y),y:x(H)})),J=s-o,ie=te.slice(0,Math.min(Math.max(J+1,1),te.length)),pe=Math.min(J+V+1,te.length),Le=J>=0&&J<te.length?te.slice(J,pe):[],ke=xi(ie),De=Le.length>=2?xi(Le):"",Fe=b(Math.max(o,Math.min(s,o+n-1))).toFixed(1),rt=ie.length?ie[ie.length-1].x.toFixed(1):Fe,_e=ke?`${ke} L${rt},${S} L${l.l},${S} Z`:"";return`<svg width="${e}" height="${i}" style="display:block;flex-shrink:0">
  ...
  `));r(p),s(p),p.querySelector("#trait-back").addEventListener("click",fe),p.querySelector(".irr-save-btn").addEventListener("click",()=>{const c=p.querySelector("#trait-date").value||n,d=p.querySelector("#trait-produit").value.trim()||"—",m=p.querySelector("#trait-dose").value.trim()||"—",h=St();h.unshift({id:Date.now(),date:c,produit:d,dose:m,plotIds:[...t]}),Pi(h),fe(),i("Traitement enregistré")})}function zo(e,i,t){const o=i==="adherent"?Ns.adherent:null,n=o!==null?le.filter(c=>c.orgId===o):le;let a=null;const s=xe(`
    ...
    <div class="w-irrig-layout">
      ...
      ${d}
      ${c}
      <button class="w-irrig-act-btn w-irrig-act-btn--sec m-irrig-view-cal" type="button"><i class="bi bi-calendar3"></i> Voir les irrigations</button>
