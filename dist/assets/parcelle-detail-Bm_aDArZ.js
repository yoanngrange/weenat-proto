...
`),`${s}_${J}_${new Date().toISOString().slice(0,10)}.csv`)}function Ce({title:e,message:t,confirmLabel:i="Confirmer",onConfirm:n}){const a=document.createElement("div");a.className="modal add-modal",a.style.zIndex="9999",a.innerHTML=`
    ...
    <div class="w-prev5-grid">${f.map(s).join("")}</div>`}function an(e){var u,d;const t=new Date,i=[{label:"Herbicides de contact",seed:7},{label:"Fongicides",seed:3},{label:"Herbicides racinaires",seed:11},{label:"Herbicides systémiques",seed:5}],n=Array.from({length:24},(f,s)=>{const r=new Date(t);return r.setMinutes(0,0,0),r.setHours(r.getHours()+s),r}),a=f=>`${String(f.getHours()).padStart(2,"0")}h`,o=f=>f.toDateString()===t.toDateString()?"Aujourd'hui":"Demain",l=f=>{const s=n.map((p,h)=>(f.seed*17+j*13+h*5)%11<6),r=s.findIndex(p=>p);if(r===-1)return null;let m=r;for(;m+1<s.length&&s[m+1];)m++;return{from:n[r],to:new Date(n[m].getTime()+36e5)}},c=i.map(f=>{const s=l(f);return`<div class="w-weephyt-family-row">
      <span class="w-weephyt-family-label">${f.label}</span>
      ${s?`<span class="w-weephyt-window"><i class="bi bi-check-circle-fill"></i> ${o(s.from)} ${a(s.from)}–${a(s.to)}</span>`:'<span class="w-weephyt-window w-weephyt-window--none">Aucune fenêtre sous 24h</span>'}
    ...
    </div>`,r=c&&!new Set(["Non irrigué","Non renseigné",""]).has(c),m=()=>{var y;const w=(y=e.closest(".dash-block"))==null?void 0:y.querySelector(".dash-block-ft-link");w&&(w.textContent=r?"Voir les irrigations →":"Gérer les irrigations →")};if(!r){e.innerHTML=`
