import{b as F,I as E,s as M}from"./irrigations-Dbn0IxRs.js";import{d as X,e as J}from"./store-DZP_FQdH.js";import{p as G}from"./plots-hjmThJP8.js";const A=new Date().toISOString().split("T")[0],K=1;X(G);const h=G.filter(i=>i.orgId===K);function C(i){const s=["jan.","fév.","mar.","avr.","mai","jun.","jul.","aoû.","sep.","oct.","nov.","déc."],[,t,a]=i.split("-");return`${parseInt(a)} ${s[parseInt(t)-1]} ${i.split("-")[0]}`}function R(i){const s=["jan","fév","mar","avr","mai","jun","jul","aoû","sep","oct","nov","déc"],[,t,a]=i.split("-");return`${parseInt(a)} ${s[parseInt(t)-1]}`}function P(i,s){return i?'<div class="iw-chk iw-chk--on"><i class="bi bi-check"></i></div>':s?'<div class="iw-chk iw-chk--partial">−</div>':'<div class="iw-chk"></div>'}function Z(i){const s=i.crop,t=i.irrigation||null,a=i.texture||null,l=[s,t,a].filter(Boolean);return l.length?`<span class="iw-plot-info">${l.join(" · ")}</span>`:`<span class="iw-plot-info iw-plot-info--miss">type d'irrigation non renseigné</span>`}function N(i,s){const t=[],a=new Set(i);for(const l of s){const e=l.ids.filter(o=>i.has(o));e.length&&(e.length===l.ids.length?(t.push(l.label),l.ids.forEach(o=>a.delete(o))):e.forEach(o=>{const d=h.find(c=>c.id===o);d&&(t.push(d.name),a.delete(o))}))}for(const l of a){const e=h.find(o=>o.id===l);e&&t.push(e.name)}return t}function ii(i,s){const t=i.size;if(t===1){const l=h.find(e=>e.id===[...i][0]);return{text:(l==null?void 0:l.name)??"Parcelle",count:null}}const a=s.find(l=>l.ids.length===t&&l.ids.every(e=>i.has(e)));return a?{text:a.label,count:a.ids.length}:{text:`${t} parcelles`,count:null}}function ti(i,s){const t=new Set(i),a=[];for(const l of s)if(l.ids.length>=2&&l.ids.every(e=>i.has(e))){const e=l.ids.map(o=>{var d;return(d=h.find(c=>c.id===o))==null?void 0:d.name}).filter(Boolean);a.push({title:`Groupe "${l.label}"`,names:e}),l.ids.forEach(o=>t.delete(o))}if(t.size>0){const l=[...t].map(e=>{var o;return(o=h.find(d=>d.id===e))==null?void 0:o.name}).filter(Boolean);a.push({title:a.length>0?"Autres parcelles":null,names:l})}return a}function ei(i){if(i.length<2)return"";const s=[...i].sort((r,b)=>r.iso<b.iso?-1:1),t=["jan","fév","mar","avr","mai","jun","jul","aoû","sep","oct","nov","déc"],a=new Date(s[0].iso),l=new Date(s[s.length-1].iso),e=Math.max(l-a,1),o=600,d=26,c=5,p=5,w=d-p-4,n=w-p-3,u=d+c+13,m=u+2,g=r=>Math.round((r-a)/e*o),q=r=>{const[,b,x]=r.split("-");return`${parseInt(x)}/${parseInt(b)}`},v=[],y=new Date(a.getFullYear(),a.getMonth()+1,1);for(;y<=l;)v.push(new Date(y)),y.setMonth(y.getMonth()+1);const f=v.map(r=>`<line x1="${g(r)}" y1="${d-6}" x2="${g(r)}" y2="${d+c+6}" stroke="#C0BEB8" stroke-width="1"/>`).join(""),k=[a,...v,l],$=k.slice(0,-1).map((r,b)=>{const x=g(r),L=g(k[b+1]);return L-x<16?"":`<text x="${Math.round((x+L)/2)}" y="${n}" font-size="9" fill="#B0AEA8" text-anchor="middle">${t[r.getMonth()]}</text>`}).join(""),S=r=>{const b=["jan","fév","mar","avr","mai","jun","jul","aoû","sep","oct","nov","déc"],[,x,L]=r.split("-");return`${parseInt(L)} ${b[parseInt(x)-1]}`},j=s.map(r=>{const b=r.real?"#FF8500":"#FFB705",x=g(new Date(r.iso)),L=`${S(r.iso)} · ${r.mm} mm`;return`<circle cx="${x}" cy="${w}" r="${p}" fill="${b}" stroke="#fff" stroke-width="1.5" pointer-events="none"/>
  ...
  </div>`}function V({title:i,params:s,labels:t,addedCount:a,filterLabel:l}){const{ov:e,close:o}=T(),d=s.map(u=>`
    ...
    `).join("")}
  </div>`}function B(i,s,t,a){i.querySelectorAll(".iw-group-card").forEach(l=>{l.addEventListener("click",()=>{const e=l.dataset.gids.split(",").map(Number),o=e.every(c=>t.has(c));e.forEach(c=>o?t.delete(c):t.add(c));const d=i.querySelector("#iw-sel-list");d&&(d.innerHTML=O(s,t)),B(i,s,t,a),a()})}),i.querySelectorAll(".iw-plot-row").forEach(l=>{l.addEventListener("click",()=>{const e=+l.dataset.pid;t.has(e)?t.delete(e):t.add(e);const o=i.querySelector("#iw-sel-list");o&&(o.innerHTML=O(s,t)),B(i,s,t,a),a()})})}function H(i=null){const s=F(h),t=new Set((i==null?void 0:i.ids)??[]);let a=A,l=10;const{ov:e,close:o}=T();e.innerHTML=`
    ...
    </div>`,c.querySelector("#iw-close").addEventListener("click",p),c.querySelector("#iw-cancel").addEventListener("click",p),c.querySelector("#iw-back-btn").addEventListener("click",()=>{d="form",q()}),c.querySelector("#iw-next-btn").addEventListener("click",()=>{d="apercu",q()}),c.querySelector("#iw-conf-btn").addEventListener("click",()=>{const v=w(),y=i?new Set(i.ids):t;Y(y,()=>{var k,$;const f=i?[i.label]:N(y,s);v.forEach(S=>{const j=S.toISOString().slice(0,10);f.forEach(r=>{E.push({iso:j,mm:e,real:j<=A,label:r,fromStrategy:!0})})}),M(),p(),V({title:"Saison enregistrée",params:[{label:"Début",value:C(((k=v[0])==null?void 0:k.toISOString().slice(0,10))??"")},{label:"Fin",value:C((($=v[v.length-1])==null?void 0:$.toISOString().slice(0,10))??"")},{label:"Quantité",value:`${e} mm/irrig.`},{label:"Fréquence",value:`tous les ${o} jours`},{label:"Irrigations générées",value:`${v.length*f.length}`}],labels:f,addedCount:v.length*f.length,filterLabel:f.length===1?f[0]:null})})}),q()}function ai(i,s){var p,w;const a=F(h).find(n=>n.label===i.label);let l=i.iso,e=i.mm,o="single";const{ov:d,close:c}=T();d.innerHTML=`
