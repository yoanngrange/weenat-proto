import{I as S,s as j,g as U}from"./irrigations-C22Dtsoo.js";import{e as W,d as X}from"./store-InOgbptE.js";import{p as _}from"./plots-BFmvZ9cD.js";import{I as J}from"./constants-C2D4LmLD.js";const M=new Date().toISOString().split("T")[0],K=1;W(_);const I=_.filter(i=>i.orgId===K);function F(i){const t=["jan.","fév.","mar.","avr.","mai","jun.","jul.","aoû.","sep.","oct.","nov.","déc."],[,e,l]=i.split("-");return`${parseInt(l)} ${t[parseInt(e)-1]} ${i.split("-")[0]}`}function O(i){const t=["jan","fév","mar","avr","mai","jun","jul","aoû","sep","oct","nov","déc"],[,e,l]=i.split("-");return`${parseInt(l)} ${t[parseInt(e)-1]}`}function N(i,t){return i?'<div class="iw-chk iw-chk--on"><i class="bi bi-check"></i></div>':'<div class="iw-chk"></div>'}function Z(i){const t=i.crop,e=i.irrigation||null,l=i.substrate?"Substrat : "+i.substrate:i.texture||null,s=[t,e,l].filter(Boolean);return s.length?`<span class="iw-plot-info">${s.join(" · ")}</span>`:`<span class="iw-plot-info iw-plot-info--miss">type d'irrigation non renseigné</span>`}function A(){return[...I].sort((i,t)=>i.name.localeCompare(t.name,"fr"))}function ii(i){if(i.length<2)return"";const t=[...i].sort((m,L)=>m.iso<L.iso?-1:1),e=["jan","fév","mar","avr","mai","jun","jul","aoû","sep","oct","nov","déc"],l=new Date(t[0].iso),s=new Date(t[t.length-1].iso),a=Math.max(s-l,1),o=600,n=26,c=5,b=5,u=n-b-4,y=u-b-3,$=n+c+13,q=$+2,E=m=>Math.round((m-l)/a*o),r=m=>{const[,L,h]=m.split("-");return`${parseInt(h)}/${parseInt(L)}`},v=[],w=new Date(l.getFullYear(),l.getMonth()+1,1);for(;w<=s;)v.push(new Date(w)),w.setMonth(w.getMonth()+1);const g=v.map(m=>`<line x1="${E(m)}" y1="${n-6}" x2="${E(m)}" y2="${n+c+6}" stroke="#C0BEB8" stroke-width="1"/>`).join(""),p=[l,...v,s],d=p.slice(0,-1).map((m,L)=>{const h=E(m),k=E(p[L+1]);return k-h<16?"":`<text x="${Math.round((h+k)/2)}" y="${y}" font-size="9" fill="#B0AEA8" text-anchor="middle">${e[m.getMonth()]}</text>`}).join(""),f=m=>{const L=["jan","fév","mar","avr","mai","jun","jul","aoû","sep","oct","nov","déc"],[,h,k]=m.split("-");return`${parseInt(k)} ${L[parseInt(h)-1]}`},x=t.map(m=>{const L=m.real?"#FF8500":"#FFB705",h=E(new Date(m.iso)),k=`${f(m.iso)} · ${m.mm} mm`;return`<circle cx="${h}" cy="${u}" r="${b}" fill="${L}" stroke="#fff" stroke-width="1.5" pointer-events="none"/>
    <circle cx="${h}" cy="${u}" r="${b+6}" fill="transparent" pointer-events="all" data-tip="${k}"/>`}).join("");return`<div class="iw-timeline">
    <svg width="${o}" height="${q}" viewBox="0 0 ${o} ${q}" style="width:100%;display:block">
      ${d}
      <rect x="0" y="${n}" width="${o}" height="${c}" rx="${c/2}" fill="#E0DED8"/>
      ${g}
      ${x}
      <text x="1" y="${$}" font-size="10" fill="#B0AEA8">${r(t[0].iso)}</text>
      <text x="${o-1}" y="${$}" font-size="10" fill="#B0AEA8" text-anchor="end">${r(t[t.length-1].iso)}</text>
    </svg>
  ...
  </div>`}function V({title:i,params:t,plotNames:e,addedCount:l,filterPlotId:s}){const{ov:a,close:o}=T(),n=t.map($=>`
    <div class="iw-conf-param">
      <span class="iw-conf-param-lbl">${$.label}</span>
      <span class="iw-conf-param-val">${$.value}</span>
    </div>`).join(""),c=5,b=e.slice(0,c),u=e.slice(c),y=b.map($=>`<span class="iw-apercu-plot-tag">${$}</span>`).join("")+(u.length?`<span class="iw-apercu-plot-more">+ ${u.length} autre${u.length>1?"s":""}</span>`:"");a.innerHTML=`
    ...
    </div>`,a.querySelector("#iw-conf-edit").addEventListener("click",()=>{S.splice(S.length-l,l),j(),o()}),a.querySelector("#iw-conf-voir").addEventListener("click",()=>{o(),Q(s??null)}),a.querySelector("#iw-conf-close").addEventListener("click",o)}function ei(i){const t=document.createElement("div");t.className="iw-toast",t.textContent=i,document.body.appendChild(t),requestAnimationFrame(()=>t.classList.add("iw-toast--in")),setTimeout(()=>{t.classList.remove("iw-toast--in"),setTimeout(()=>t.remove(),400)},3e3)}function Y(i,t){const e=I.filter(o=>i.has(o.id)&&(!o.irrigation||o.irrigation==="Non irrigué"));if(!e.length){t();return}const{ov:l,close:s}=T();l.innerHTML=`
  ...
  </div>`}function D(i,t,e){var l;i.querySelectorAll(".iw-plot-row").forEach(s=>{s.addEventListener("click",()=>{const a=+s.dataset.pid;t.has(a)?t.delete(a):t.add(a);const o=i.querySelector("#iw-sel-list");o&&(o.innerHTML=B(t)),D(i,t,e),e()})}),(l=i.querySelector("#iw-sel-all"))==null||l.addEventListener("click",()=>{const s=A(),a=s.some(n=>!t.has(n.id));s.forEach(n=>a?t.add(n.id):t.delete(n.id));const o=i.querySelector("#iw-sel-list");o&&(o.innerHTML=B(t)),i.querySelector("#iw-sel-all").textContent=a?"Tout désélectionner":"Tout sélectionner",D(i,t,e),e()})}function H(i=null){const t=new Set((i==null?void 0:i.ids)??[]);let e=M,l=10;const{ov:s,close:a}=T();s.innerHTML=`
    <div class="iw-modal iw-modal--wide">
    ...
    </div>`,s.querySelector("#iw-close").addEventListener("click",a),s.querySelector("#iw-cancel").addEventListener("click",a),s.querySelector("#iw-date").addEventListener("change",n=>{e=n.target.value}),s.querySelector("#iw-qty").addEventListener("input",n=>{l=parseInt(n.target.value)||1});function o(){s.querySelector("#iw-save").disabled=!i&&t.size===0;const n=s.querySelector("#iw-sel-sum");n&&(n.innerHTML=z(t))}i||D(s,t,o),s.querySelector("#iw-save").addEventListener("click",()=>{const n=i?new Set(i.ids):t;Y(n,()=>{const c=e>M;n.forEach(u=>{S.push({iso:e,mm:l,real:!c,plotId:u,fromStrategy:!1})});const b=[...n].map(u=>{var y;return(y=I.find($=>$.id===u))==null?void 0:y.name}).filter(Boolean);j(),a(),V({title:n.size>1?"Irrigations enregistrées":"Irrigation enregistrée",params:[{label:"Date",value:F(e)},{label:"Quantité",value:`${l} mm`}],plotNames:b,addedCount:n.size,filterPlotId:n.size===1?[...n][0]:null})})})}function C(i=null){const t=new Set((i==null?void 0:i.ids)??[]);let e=M,l=new Date(new Date().setMonth(new Date().getMonth()+4)).toISOString().split("T")[0],s=10,a=7,o="form";const{ov:n,close:c}=T();function b(){if(!e||!l||a<=0)return[];const r=[],v=new Date(l);let w=new Date(e);for(;w<=v&&r.length<100;)r.push(new Date(w)),w.setDate(w.getDate()+a);return r}function u(){const r=b(),v=r.length>0?`<div class="iw-preview-ok">↗ ${r.length} irrigations · ${r.length*s} mm au total</div>`:'<div class="iw-preview-empty">Ajustez les dates et la fréquence</div>';return`
      <div class="iw-two-col">
        <div class="iw-panel iw-panel--params">
        ...
        ${G(t,i)}
      </div>`}function y(){const r=b(),v=r.filter(h=>h.toISOString().slice(0,10)<M),w=r.filter(h=>h.toISOString().slice(0,10)>=M),g=i?i.ids.length:t.size;function p(h){const k=h.toISOString().slice(0,10),P=k>=M?"#FFB705":"#FF8500";return`<div class="iw-occ-row">
        <span>${F(k)}</span>
        <span style="font-weight:700;color:${P}">${s} mm</span>
      </div>`}const d=i?I.filter(h=>i.ids.includes(h.id)):I.filter(h=>t.has(h.id)),f=4,x=d.slice(0,f),m=d.slice(f),L=`
      <div class="iw-apercu-plots">
        <div class="iw-section-lbl">Parcelles concernées</div>
        ${x.map(h=>`<span class="iw-apercu-plot-tag">${h.name}</span>`).join("")}${m.length?`<span class="iw-apercu-plot-more">+ ${m.length} autre${m.length>1?"s":""}</span>`:""}
      </div>`;return`
      <div class="iw-apercu-summary">
        ...
        <div style="text-align:right">
          ...
          <div class="iw-apercu-sub">${g} parcelle${g>1?"s":""} · ${s} mm/irrig.</div>
        </div>
      </div>
      ${L}
    ...
    `}function $(){const r=i?i.ids.length:t.size,v=b(),w=n.querySelector("#iw-next-btn");w&&(w.disabled=r===0||v.length===0)}function q(){var v,w,g,p;i||D(n,t,$);const r=()=>{const d=b(),f=n.querySelector(".iw-preview-ok, .iw-preview-empty");f&&(f.className=d.length>0?"iw-preview-ok":"iw-preview-empty",f.textContent=d.length>0?`↗ ${d.length} irrigations · ${d.length*s} mm au total`:"Ajustez les dates et la fréquence"),$()};(v=n.querySelector("#iw-debut"))==null||v.addEventListener("change",d=>{e=d.target.value,r()}),(w=n.querySelector("#iw-fin"))==null||w.addEventListener("change",d=>{l=d.target.value,r()}),(g=n.querySelector("#iw-qty"))==null||g.addEventListener("input",d=>{s=parseInt(d.target.value)||1,r()}),(p=n.querySelector("#iw-freq"))==null||p.addEventListener("input",d=>{a=parseInt(d.target.value)||1,r()})}function E(){var v;const r=n.querySelector(".iw-modal");r.querySelector(".iw-modal-body").innerHTML=o==="form"?u():y(),r.querySelector("#iw-back-btn").style.display=o==="form"?"none":"",r.querySelector("#iw-next-btn").style.display=o==="form"?"":"none",r.querySelector("#iw-conf-btn").style.display=o==="form"?"none":"",o==="form"?($(),q()):(v=n.querySelector("#iw-apercu-more"))==null||v.addEventListener("click",w=>{n.querySelector("#iw-apercu-rest").style.display="",w.target.remove()})}n.innerHTML=`
    <div class="iw-modal iw-modal--wide">
      <div class="iw-modal-hd">
    ...
    </div>`,n.querySelector("#iw-close").addEventListener("click",c),n.querySelector("#iw-cancel").addEventListener("click",c),n.querySelector("#iw-back-btn").addEventListener("click",()=>{o="form",E()}),n.querySelector("#iw-next-btn").addEventListener("click",()=>{o="apercu",E()}),n.querySelector("#iw-conf-btn").addEventListener("click",()=>{const r=b(),v=i?new Set(i.ids):t;Y(v,()=>{var p,d;const w=U();r.forEach(f=>{const x=f.toISOString().slice(0,10);v.forEach(m=>{S.push({iso:x,mm:s,real:x<=M,plotId:m,fromStrategy:!0,seasonId:w})})});const g=[...v].map(f=>{var x;return(x=I.find(m=>m.id===f))==null?void 0:x.name}).filter(Boolean);j(),c(),V({title:"Saison enregistrée",params:[{label:"Début",value:F(((p=r[0])==null?void 0:p.toISOString().slice(0,10))??"")},{label:"Fin",value:F(((d=r[r.length-1])==null?void 0:d.toISOString().slice(0,10))??"")},{label:"Quantité",value:`${s} mm/irrig.`},{label:"Fréquence",value:`tous les ${a} jours`},{label:"Irrigations générées",value:`${r.length*v.size}`}],plotNames:g,addedCount:r.length*v.size,filterPlotId:v.size===1?[...v][0]:null})})}),E()}function si(i,t){let e=i.iso,l=i.mm;const{ov:s,close:a}=T();s.innerHTML=`
    <div class="iw-modal" style="max-width:420px;width:95%">
    ...
    </div>`,s.querySelector("#iw-close").addEventListener("click",a),s.querySelector("#iw-cancel").addEventListener("click",a),s.querySelector("#iw-edit-date").addEventListener("change",o=>{e=o.target.value}),s.querySelector("#iw-edit-qty").addEventListener("input",o=>{const n=parseInt(o.target.value);n>0&&(l=n)}),s.querySelector("#iw-delete").addEventListener("click",()=>{const o=S.indexOf(i);o>-1&&S.splice(o,1),j(),a(),t()}),s.querySelector("#iw-save").addEventListener("click",()=>{i.iso=e,i.mm=l,i.real=e<=M,j(),a(),t()})}function R(i,t){let e=!1,l=!1;const{ov:s,close:a}=T();function o(){return new Set(S.filter(c=>c.plotId===i&&c.fromStrategy&&c.seasonId).map(c=>c.seasonId))}function n(){var y,$,q,E,r,v,w;const c=s.querySelector(".iw-modal-body");c.innerHTML=`
      ...
      </div>`:""}`;const b=I.find(g=>g.id===i),u=b?{ids:[b.id],label:b.name}:null;(y=c.querySelector("#strat-modify"))==null||y.addEventListener("click",()=>{a(),C(u)}),($=c.querySelector("#strat-stop"))==null||$.addEventListener("click",()=>{e=!0,l=!1,n()}),(q=c.querySelector("#strat-delete"))==null||q.addEventListener("click",()=>{l=!0,e=!1,n()}),(E=c.querySelector("#stop-cancel"))==null||E.addEventListener("click",()=>{e=!1,n()}),(r=c.querySelector("#stop-confirm"))==null||r.addEventListener("click",()=>{const g=o();S.splice(0,S.length,...S.filter(p=>!(p.plotId===i&&g.has(p.seasonId)&&!p.real))),j(),a(),t()}),(v=c.querySelector("#del-cancel"))==null||v.addEventListener("click",()=>{l=!1,n()}),(w=c.querySelector("#del-confirm"))==null||w.addEventListener("click",()=>{const g=o();S.splice(0,S.length,...S.filter(p=>!(p.plotId===i&&g.has(p.seasonId)))),j(),a(),t()})}s.innerHTML=`
    ...
    </div>`,s.querySelector("#iw-close").addEventListener("click",a),s.querySelector("#iw-cancel").addEventListener("click",a),n()}function ni(){var c,b;const i=new Set(I.map(u=>u.id)),t=[...S].sort((u,y)=>u.iso<y.iso?-1:1);let e=((c=t[0])==null?void 0:c.iso)??M,l=((b=t[t.length-1])==null?void 0:b.iso)??M;const{ov:s,close:a}=T();function o(){return A().map(u=>{const y=i.has(u.id);return`<div class="iw-plot-row${y?" iw-plot-row--sel":""}" data-pid="${u.id}">
        ${N(y)}
    ...
    </div>`,s.querySelector("#iw-close").addEventListener("click",a),s.querySelector("#iw-cancel").addEventListener("click",a),s.querySelector("#iw-exp-start").addEventListener("change",u=>{e=u.target.value}),s.querySelector("#iw-exp-end").addEventListener("change",u=>{l=u.target.value});function n(){s.querySelectorAll("#iw-export-plots .iw-plot-row").forEach(u=>{u.addEventListener("click",()=>{const y=+u.dataset.pid;i.has(y)?i.delete(y):i.add(y),s.querySelector("#iw-export-plots").innerHTML=o(),n()})})}n(),s.querySelector("#iw-do-export").addEventListener("click",()=>{const u=A().filter(p=>i.has(p.id));if(!u.length){ei("Sélectionnez au moins une parcelle");return}const y=new Set;for(const p of S)p.iso>=e&&p.iso<=l&&y.add(p.iso);const $=[...y].sort(),q=["Date",...u.map(p=>p.name)].join(";"),E=$.map(p=>{const d=u.map(f=>{const x=S.filter(m=>m.iso===p&&m.plotId===f.id).reduce((m,L)=>m+L.mm,0);return x>0?x:""});return[p,...d].join(";")}),r=[q,...E].join(`
`),v=new Blob([r],{type:"text/csv;charset=utf-8;"}),w=URL.createObjectURL(v),g=document.createElement("a");g.href=w,g.download=`irrigations_${e}_${l}.csv`,document.body.appendChild(g),g.click(),setTimeout(()=>{URL.revokeObjectURL(w),g.remove()},100),a()})}function Q(i=null){let t=i?String(i):"all";const{ov:e,close:l}=T();function s(){return t==="all"?S:S.filter(c=>c.plotId===+t)}function a(){if(t==="all")return null;const c=I.find(b=>String(b.id)===t);return c?{ids:[c.id],label:c.name}:null}const o=A().map(c=>`<option value="${c.id}"${String(c.id)===t?" selected":""}>${c.name}</option>`).join("");e.innerHTML=`
    <div class="iw-modal iw-modal--full">
      ...
      <div class="iw-full-body" id="iw-full-body"></div>
    </div>`,e.querySelector("#iw-close").addEventListener("click",l),e.querySelector("#iw-filter").addEventListener("change",c=>{t=c.target.value,n()}),e.querySelector("#iw-export-btn").addEventListener("click",ni);function n(){var w,g,p;const c=e.querySelector("#iw-full-body"),b=s(),u=b.some(d=>d.fromStrategy),y=e.querySelector("#iw-strat-btn");if(y.style.display=u&&t!=="all"?"":"none",y.onclick=()=>R(+t,n),!b.length){c.innerHTML=`
        <div class="iw-empty">
        ...
        </div>`,c.querySelector("#iw-add-saisie").addEventListener("click",()=>{l(),H(a())}),c.querySelector("#iw-add-saison").addEventListener("click",()=>{l(),C(a())});return}const $={};c.querySelectorAll("details[data-section]").forEach(d=>{$[d.dataset.section]=d.open});const q=[...b].filter(d=>!d.real).sort((d,f)=>d.iso<f.iso?-1:1),E=[...b].filter(d=>d.real).sort((d,f)=>d.iso>f.iso?-1:1);function r(d){var k;const f=S.indexOf(d),x=d.real?"#FF8500":"#FFB705",[,m,L]=d.iso.split("-"),h=((k=I.find(P=>P.id===d.plotId))==null?void 0:k.name)??"—";return`<div class="iw-list-item">
        <div class="iw-list-stripe" style="background:${x}"></div>
        <div class="iw-list-date"><span style="font-size:15px;font-weight:700;color:${x}">${L}</span><span style="font-size:10px;color:${x}">/${m}</span></div>
        <div style="flex:1">
        ...
        <button class="iw-edit-btn" data-iidx="${f}">✎</button>
        <button class="iw-del-btn"  data-iidx="${f}">🗑</button>
      ...
      ${E.length?`
        <details class="iw-section-details" data-section="effectuees" open>
          <summary class="iw-section-summary">Effectuées <span class="iw-section-count">${E.length}</span></summary>
          <div class="iw-section-body">${E.map(r).join("")}</div>
        </details>`:""}
      ${q.length?`
        <details class="iw-section-details" data-section="planifiees" open>
          ...
          <div class="iw-section-body">${q.map(r).join("")}</div>
        </details>`:""}
    `,c.querySelectorAll("details[data-section]").forEach(d=>{const f=d.dataset.section;f in $&&(d.open=$[f])}),c.querySelectorAll(".iw-edit-btn").forEach(d=>{d.addEventListener("click",()=>{const f=S[+d.dataset.iidx];f&&si(f,n)})}),c.querySelectorAll(".iw-del-btn").forEach(d=>{d.addEventListener("click",()=>{const f=+d.dataset.iidx;f>=0&&(S.splice(f,1),j(),n())})}),(w=c.querySelector("#iw-add-saisie"))==null||w.addEventListener("click",()=>{l(),H(a())}),(g=c.querySelector("#iw-add-saison"))==null||g.addEventListener("click",()=>{l(),C(a())}),(p=c.querySelector("#iw-strat-inline"))==null||p.addEventListener("click",()=>R(+t,n))}n()}(()=>{const i=Object.assign(document.createElement("div"),{id:"irr-data-tip",className:"irr-data-tip"});document.body.appendChild(i),document.addEventListener("mouseover",t=>{const e=t.target.closest("[data-tip]");if(!e){i.style.opacity="0";return}i.textContent=e.dataset.tip,i.style.opacity="1"}),document.addEventListener("mousemove",t=>{i.style.left=t.clientX+14+"px",i.style.top=t.clientY-36+"px"}),document.addEventListener("mouseout",t=>{var e;(e=t.relatedTarget)!=null&&e.closest("[data-tip]")||(i.style.opacity="0")})})();window.WebIrrig={openSaisie:H,openSaison:C,openVoirIrrigations:Q};
