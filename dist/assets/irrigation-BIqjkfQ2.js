import"./modulepreload-polyfill-B5Qt9EMX.js";/* empty css             */import{I as d,R as U,s as A,b as ve}from"./irrigations-Dbn0IxRs.js";import{p as J}from"./plots-hjmThJP8.js";import{o as me}from"./orgs-Cx4wNzLL.js";import{u as fe}from"./breadcrumb-DEBQjfiD.js";import"./irrigation-web-BFPaoYvD.js";import"./store-DZP_FQdH.js";const ge=1,ae=(localStorage.getItem("menuRole")||"admin-reseau")==="admin-reseau";let z=null,M=ae?J:J.filter(e=>e.orgId===ge);const D=new Date().toISOString().split("T")[0];let R=null,H=0;function re(e,t){const r=new Date(e);return r.setDate(r.getDate()+t),r.toISOString().split("T")[0]}function Q(e){const t=["jan.","fév.","mar.","avr.","mai","jun.","jul.","aoû.","sep.","oct.","nov.","déc."],[,r,i]=e.split("-");return`${parseInt(i)} ${t[parseInt(r)-1]} ${e.split("-")[0]}`}function _(e){const t=["jan","fév","mar","avr","mai","jun","jul","aoû","sep","oct","nov","déc"],[,r,i]=e.split("-");return`${parseInt(i)} ${t[parseInt(r)-1]}`}function be(e){const[,t,r]=e.split("-");return`${r}/${t}`}function B(){return ve(M)}function Z(e){if(!e||e==="all")return null;const t=B().find(i=>i.label===e);if(t)return{ids:t.ids,label:t.label};const r=M.find(i=>String(i.id)===String(e));return r?{ids:[r.id],label:r.name}:null}function ye(e){if(!e)return d;const t=B().find(i=>i.label===e.label);if(t)return d.filter(i=>i.label===t.label);const r=M.find(i=>i.name===e.label);if(r){const i=new Set(B().filter(p=>p.ids.includes(r.id)).map(p=>p.label));return d.filter(p=>p.label===r.name||i.has(p.label))}return d}function ne(e){const t=new Set([e.name]);for(const r of B())r.ids.includes(e.id)&&t.add(r.label);return t}function se(e,t){var p;(p=document.querySelector(".irr-save-confirm"))==null||p.remove();const r=document.createElement("div");r.className="irr-save-confirm",r.innerHTML=`
    ...
    ${p}${n}
  </select>`}function oe(e){if(e.length<2)return"";const t=[...e].sort((o,q)=>o.iso<q.iso?-1:1),r=["jan","fév","mar","avr","mai","jun","jul","aoû","sep","oct","nov","déc"],i=new Date(t[0].iso),p=new Date(t[t.length-1].iso),n=Math.max(p-i,1),c=500,b=24,w=5,L=5,k=b-L-4,j=k-L-3,v=b+w+13,u=v+2,y=o=>Math.round((o-i)/n*c),S=o=>{const[,q,s]=o.split("-");return`${parseInt(s)}/${parseInt(q)}`},g=[],l=new Date(i.getFullYear(),i.getMonth()+1,1);for(;l<=p;)g.push(new Date(l)),l.setMonth(l.getMonth()+1);const m=g.map(o=>`<line x1="${y(o)}" y1="${b-6}" x2="${y(o)}" y2="${b+w+6}" stroke="#C0BEB8" stroke-width="1"/>`).join(""),$=[i,...g,p],x=$.slice(0,-1).map((o,q)=>{const s=y(o),f=y($[q+1]);return f-s<16?"":`<text x="${Math.round((s+f)/2)}" y="${j}" font-size="9" fill="#B0AEA8" text-anchor="middle">${r[o.getMonth()]}</text>`}).join(""),a=t.map(o=>{const q=o.real?"#FF8500":"#FFB705",s=y(new Date(o.iso)),f=`${_(o.iso)} · ${o.mm} mm`;return`<circle cx="${s}" cy="${k}" r="${L}" fill="${q}" stroke="#fff" stroke-width="1.5" pointer-events="none"/>
    <circle cx="${s}" cy="${k}" r="${L+6}" fill="transparent" pointer-events="all" data-tip="${f}"/>`}).join("");return`<div class="irr-pg-timeline">
    <svg width="${c}" height="${u}" viewBox="0 0 ${c} ${u}" style="width:100%;display:block">
      ${x}<rect x="0" y="${b}" width="${c}" height="${w}" rx="${w/2}" fill="#E0DED8"/>${m}${a}
  ...
  </div>`}function O(e=!1){var a,o,q;const t=document.getElementById("irr-right"),r=re(D,14),i=[...d.map(s=>s.iso),...U.filter(s=>s.iso<=r).map(s=>s.iso),r];if(!d.length&&!U.length){t.innerHTML=`<div class="irr-rv-hd">Vue globale</div><div class="irr-pr-empty"><i class="bi bi-table"></i><p>Il manque des données de pluie ou d'irrigations sur au moins une parcelle pour afficher cette vue</p></div>`;return}const p=[...i].sort()[0],n=d.length?[...d].map(s=>s.iso).sort().at(-1):null,c=n&&n>r?n:r,b=new Date(c);b.setMonth(b.getMonth()+H);const w=b.toISOString().split("T")[0],L=e?null:((a=t.querySelector("#irr-gl-body"))==null?void 0:a.scrollLeft)??null,k=[];let j=new Date(p);const v=new Date(w);for(;j<=v;)k.push(j.toISOString().split("T")[0]),j.setDate(j.getDate()+1);const u={};for(const s of U)u[s.iso]=s.mm;function y(s,f,E){if(!E)return 0;const[,I,h]=f.split("-").map(Number);if((s*13+I*7+h*3)%100<12)return 0;const C=.75+(s*11+h*5)%50/100;return Math.round(E*C)}const S=k.map(s=>`<th class="irr-gl-th${s===D?" irr-gl-th--today":""}">${be(s)}</th>`).join("");function g(s){const f=ne(s),E=k.map(I=>{const h=I===D,T=d.filter(F=>F.iso===I&&f.has(F.label)),C=T.reduce((F,K)=>F+K.mm,0),V=T.some(F=>F.real),G=(s.id*7+3)%3-1,N=re(I,G),Y=y(s.id,I,u[N]??0);let P="";if(C){const F=V?"#E07820":"#FFB705",K=T.map(pe=>d.indexOf(pe)).join(",");P+=`<span class="irr-gl-val irr-gl-irrig" data-idxs="${K}" data-iso="${I}" style="color:${F};cursor:pointer">${C} mm</span>`}return Y&&(P+=`<span class="irr-gl-rain">${Y} mm</span>`),C||(P+=`<button class="irr-gl-add" data-plot-id="${s.id}" data-plot-name="${s.name}" data-iso="${I}">+</button>`),`<td class="irr-gl-td${h?" irr-gl-td--today":""}">${P}</td>`}).join("");return`<tr data-row-plot="${s.id}"><td class="irr-gl-plot"><span class="irr-gl-plot-link" data-plot-id="${s.id}">${s.name}</span></td>${E}</tr>`}const l=B(),m=M.filter(s=>!l.some(f=>f.ids.includes(s.id))),$=k.map(s=>`<td class="irr-gl-td${s===D?" irr-gl-td--today":""}"></td>`).join(""),x=[...l.map(s=>{const f=s.ids.map(I=>M.find(h=>h.id===I)).filter(Boolean);return`<tr class="irr-gl-group-row" data-row-group="${s.label}"><td class="irr-gl-group-cell"><span class="irr-gl-group-link" data-group-label="${s.label}">⬡ ${s.label}</span></td>${$}</tr>`+f.map(g).join("")}),m.length?`<tr class="irr-gl-group-row"><td class="irr-gl-group-cell irr-gl-group-cell--other">Autres parcelles</td>${$}</tr>`+m.map(g).join(""):""].join("");t.innerHTML=`
    ...
    </div>`,requestAnimationFrame(()=>{var h;const s=t.querySelector("#irr-gl-body"),f=t.querySelector("#irr-gl-table"),E=t.querySelector("#irr-gl-scroll-top"),I=t.querySelector("#irr-gl-scroll-inner");if(!(!s||!f||!E||!I))if(I.style.width=f.offsetWidth+"px",E.addEventListener("scroll",()=>{s.scrollLeft=E.scrollLeft}),s.addEventListener("scroll",()=>{E.scrollLeft=s.scrollLeft}),L!==null)s.scrollLeft=L,E.scrollLeft=L;else{const T=t.querySelector(".irr-gl-th--today");if(T){const V=T.previousElementSibling??T,G=((h=t.querySelector(".irr-gl-plot-hd"))==null?void 0:h.offsetWidth)??120,N=V.offsetLeft-G;s.scrollLeft=N,E.scrollLeft=N}}}),(o=t.querySelector("#irr-gl-prev-month"))==null||o.addEventListener("click",()=>{H>0&&(H--,O())}),(q=t.querySelector("#irr-gl-next-month"))==null||q.addEventListener("click",()=>{H++,O()}),t.querySelectorAll(".irr-gl-irrig").forEach(s=>{s.addEventListener("click",()=>{he(s.dataset.idxs.split(",").map(Number),s.dataset.iso)})}),t.querySelectorAll(".irr-gl-add").forEach(s=>{s.addEventListener("click",()=>{$e(s.dataset.plotId,s.dataset.plotName,s.dataset.iso)})}),t.querySelectorAll(".irr-gl-plot-link").forEach(s=>{s.addEventListener("click",()=>{const f=M.find(E=>String(E.id)===s.dataset.plotId);f&&de(f)})}),t.querySelectorAll(".irr-gl-group-link").forEach(s=>{s.addEventListener("click",()=>{const f=B().find(E=>E.label===s.dataset.groupLabel);f&&W(f)})})}function he(e,t){var p;(p=document.querySelector(".irr-edit-overlay"))==null||p.remove();const r=document.createElement("div");r.className="irr-edit-overlay";const i=e.map(n=>d[n]).filter(Boolean);r.innerHTML=`
    <div class="irr-edit-modal">
      <div class="irr-edit-hd">
    ...
    <div class="irr-det-sticky">
      ...
      <div class="irr-det-body">
        <div class="irr-det-meta">
          ${c&&b?`<div><strong>Saison :</strong> ${_(c)} → ${_(b)}</div>`:'<div style="color:var(--txt3)">Pas de saison enregistrée</div>'}
        </div>
        ${j}
        ${ce(i)}
        ${oe(i)}
    ...
    <div class="irr-det-scroll">
      ${w.length?`<details class="irr-pr-details"><summary class="irr-pr-details-sum"><i class="bi bi-chevron-right irr-pr-caret"></i>Effectuées <span class="irr-pr-details-sub">${w.length}</span></summary><div>${w.map(k).join("")}</div></details>`:""}
      ${L.length?`<details class="irr-pr-details" open><summary class="irr-pr-details-sum"><i class="bi bi-chevron-right irr-pr-caret"></i>Planifiées <span class="irr-pr-details-sub">${L.length}</span></summary><div>${L.map(k).join("")}</div></details>`:""}
      ${i.length?"":'<div class="irr-pr-empty" style="padding:24px"><i class="bi bi-calendar3"></i><p>Aucune irrigation pour ce groupe</p></div>'}
    </div>`,(y=r.querySelector("#irr-det-strat-toggle"))==null||y.addEventListener("click",()=>{const l=r.querySelector("#irr-det-strat-opts");l.style.display=l.style.display==="none"?"":"none"}),(S=r.querySelector("#irr-det-stop"))==null||S.addEventListener("click",()=>{d.splice(0,d.length,...d.filter(l=>!(l.label===e.label&&!l.real&&l.iso>D))),A(),W(e),O(!0)}),(g=r.querySelector("#irr-det-del-all"))==null||g.addEventListener("click",()=>{d.splice(0,d.length,...d.filter(l=>l.label!==e.label)),A(),W(e),O(!0)}),r.querySelectorAll(".irr-pr-del").forEach(l=>{l.addEventListener("click",()=>{const m=+l.dataset.idx;m>=0&&(d.splice(m,1),A(),W(e))})}),r.querySelector("#irr-det-close").addEventListener("click",()=>{var l;(l=document.getElementById("irr-page"))==null||l.classList.remove("irr-has-detail"),r.style.display="none",r.dataset.plotId=""})}function de(e){const t=document.getElementById("irr-page"),r=document.getElementById("irr-detail");if(!(!t||!r)){if(r.dataset.plotId===String(e.id)&&t.classList.contains("irr-has-detail")){t.classList.remove("irr-has-detail"),r.style.display="none",r.dataset.plotId="";return}r.dataset.plotId=String(e.id),r.style.display="",t.classList.add("irr-has-detail"),ie(String(e.id),null),X(e)}}const ue=["jan","fév","mar","avr","mai","jun","jul","aoû","sep","oct","nov","déc"];function X(e){var g,l,m,$,x;const t=document.getElementById("irr-detail");if(document.getElementById("irr-page"),!t)return;const i=B().find(a=>a.ids.includes(e.id)),p={ids:[e.id],label:e.name},n=ye(p),c=n.some(a=>a.fromStrategy),b=n.filter(a=>a.fromStrategy).sort((a,o)=>a.iso<o.iso?-1:1),w=(g=b[0])==null?void 0:g.iso,L=(l=b[b.length-1])==null?void 0:l.iso,k=i?`<div><strong>Groupe :</strong> ${i.label} — ${i.ids.length} parcelles</div>`:"",j=w&&L?`<div><strong>Saison :</strong> ${_(w)} → ${_(L)}</div>`:'<div style="color:var(--txt3)">Pas de saison enregistrée</div>',v=n.filter(a=>a.real).sort((a,o)=>a.iso>o.iso?-1:1),u=n.filter(a=>!a.real).sort((a,o)=>a.iso<o.iso?-1:1),y=a=>{const o=d.indexOf(a),q=a.real?"#FF8500":"#FFB705",[,s,f]=a.iso.split("-"),E=`${parseInt(f)} ${ue[parseInt(s)-1]}`;return`<div class="irr-pr-item">
    ...
    </div>`,document.body.appendChild(i),i.addEventListener("click",n=>{n.target===i&&i.remove()}),i.querySelector("#irr-qa-close").addEventListener("click",()=>i.remove()),i.querySelector("#irr-qa-cancel").addEventListener("click",()=>i.remove()),i.querySelector("#irr-qa-save").addEventListener("click",()=>{const n=parseInt(i.querySelector("#irr-qa-qty").value)||10,c=M.find(b=>String(b.id)===String(e));c&&(d.push({iso:r,mm:n,real:r<=D,label:c.name,fromStrategy:!1}),A()),i.remove()})}function te(){var p,n,c,b,w,L,k,j;const e=document.getElementById("irr-left"),t=[...me].filter(v=>v.id!==100).sort((v,u)=>v.name.localeCompare(u.name)),r=ae?`
  ...
  `,(p=e.querySelector("#irr-org-select"))==null||p.addEventListener("change",v=>{const u=v.target.value;z=u===""?null:parseInt(u),M=z===null?J:J.filter(y=>y.orgId===z),H=0,te(),O()}),e.querySelectorAll(".irr-lc-btn").forEach(v=>{v.addEventListener("click",()=>{const u=v.dataset.action;R=R===u?null:u,te()})});function i(){var x,a,o,q;const v=(x=e.querySelector("#irr-sa-debut"))==null?void 0:x.value,u=(a=e.querySelector("#irr-sa-fin"))==null?void 0:a.value,y=parseInt((o=e.querySelector("#irr-sa-qty"))==null?void 0:o.value)||0,S=parseInt((q=e.querySelector("#irr-sa-freq"))==null?void 0:q.value)||0,g=e.querySelector("#irr-sa-preview");if(!g)return;if(!v||!u||S<=0){g.textContent="—";return}let l=0,m=new Date(v),$=new Date(u);for(;m<=$&&l<200;)l++,m.setDate(m.getDate()+S);g.innerHTML=l>0?`<span style="color:var(--pri);font-weight:600">↗ ${l} irrigations · ${l*y} mm au total</span>`:'<span style="color:var(--txt3)">Ajustez les paramètres</span>'}R==="saison"&&(i(),(n=e.querySelector("#irr-sa-debut"))==null||n.addEventListener("change",i),(c=e.querySelector("#irr-sa-fin"))==null||c.addEventListener("change",i),(b=e.querySelector("#irr-sa-qty"))==null||b.addEventListener("input",i),(w=e.querySelector("#irr-sa-freq"))==null||w.addEventListener("input",i)),(L=e.querySelector("#irr-ex-dl"))==null||L.addEventListener("click",()=>{const v=e.querySelector("#irr-ex-scope").value,u=e.querySelector("#irr-ex-start").value,y=e.querySelector("#irr-ex-end").value,S=e.querySelector("#irr-ex-irrig").checked,g=e.querySelector("#irr-ex-rain").checked,l=Z(v),m=l?M.filter(h=>l.ids.includes(h.id)):M,$=new Set;if(S)for(const h of d)h.iso>=u&&h.iso<=y&&$.add(h.iso);if(g)for(const h of U)h.iso>=u&&h.iso<=y&&$.add(h.iso);const x=[...$].sort(),a={};if(g)for(const h of U)a[h.iso]=h.mm;const o=["Date",...m.map(h=>h.name+(g?" (irrig.)":""))].join(";"),q=x.map(h=>{const T=m.map(C=>{const V=ne(C),G=S?d.filter(P=>P.iso===h&&V.has(P.label)).reduce((P,F)=>P+F.mm,0):0,N=g?a[h]??0:0,Y=G+N;return Y>0?Y:""});return[h,...T].join(";")}),s=[o,...q].join(`
`),f=new Blob([s],{type:"text/csv;charset=utf-8;"}),E=URL.createObjectURL(f),I=Object.assign(document.createElement("a"),{href:E,download:`irrigations_${u}_${y}.csv`});document.body.appendChild(I),I.click(),setTimeout(()=>{URL.revokeObjectURL(E),I.remove()},100)}),(k=e.querySelector("#irr-s-save"))==null||k.addEventListener("click",()=>{const v=e.querySelector("#irr-s-scope").value,u=e.querySelector("#irr-s-date").value,y=parseInt(e.querySelector("#irr-s-qty").value)||0,S=e.querySelector("#irr-s-err");if(!v||!u||!y){S&&(S.style.display="");return}S&&(S.style.display="none");const g=Z(v),l=u>D,m=g?[g.label]:[],$=()=>{m.forEach(a=>d.push({iso:u,mm:y,real:!l,label:a,fromStrategy:!1})),A(),se("Irrigation enregistrée",[`Date : ${Q(u)}`,`Quantité : ${y} mm`,`Périmètre : ${(g==null?void 0:g.label)??"Toutes les parcelles"}`])},x=d.filter(a=>m.includes(a.label));if(x.length){le(x.length,(g==null?void 0:g.label)??"Toutes les parcelles",()=>{d.splice(0,d.length,...d.filter(a=>!m.includes(a.label))),$()});return}$()}),(j=e.querySelector("#irr-sa-save"))==null||j.addEventListener("click",()=>{const v=e.querySelector("#irr-sa-scope").value,u=e.querySelector("#irr-sa-debut").value,y=e.querySelector("#irr-sa-fin").value,S=parseInt(e.querySelector("#irr-sa-qty").value)||10,g=parseInt(e.querySelector("#irr-sa-freq").value)||7,l=e.querySelector("#irr-sa-err");if(!v||!u||!y){l&&(l.style.display="");return}l&&(l.style.display="none");const m=Z(v),$=m?[m.label]:[],x=[];let a=new Date(u),o=new Date(y);for(;a<=o&&x.length<200;)x.push(new Date(a)),a.setDate(a.getDate()+g);const q=()=>{$.forEach(f=>x.forEach(E=>{const I=E.toISOString().slice(0,10);d.push({iso:I,mm:S,real:I<=D,label:f,fromStrategy:!0})})),A(),se("Saison enregistrée",[`Début : ${Q(u)} · Fin : ${Q(y)}`,`${x.length} irrigations · ${S} mm · tous les ${g} j`,`Périmètre : ${(m==null?void 0:m.label)??"Toutes les parcelles"}`])},s=d.filter(f=>$.includes(f.label)&&f.iso>=u&&f.iso<=y);if(s.length){le(s.length,(m==null?void 0:m.label)??"Toutes les parcelles",()=>{d.splice(0,d.length,...d.filter(f=>!($.includes(f.label)&&f.iso>=u&&f.iso<=y))),q()});return}q()})}function Se(){const e=Object.assign(document.createElement("div"),{id:"irr-data-tip",className:"irr-data-tip"});document.body.appendChild(e),document.addEventListener("mouseover",t=>{const r=t.target.closest("[data-tip]");if(!r){e.style.opacity="0";return}e.textContent=r.dataset.tip,e.style.opacity="1"}),document.addEventListener("mousemove",t=>{e.style.left=t.clientX+14+"px",e.style.top=t.clientY-36+"px"}),document.addEventListener("mouseout",t=>{var r;(r=t.relatedTarget)!=null&&r.closest("[data-tip]")||(e.style.opacity="0")})}document.addEventListener("DOMContentLoaded",()=>{fe(),Se(),te(),O(),window.addEventListener("irrig-updated",()=>{O()});const t=new URLSearchParams(window.location.search).get("plot");if(t){const r=M.find(i=>i.id===+t);r&&de(r)}});
