import"./modulepreload-polyfill-B5Qt9EMX.js";/* empty css             */import{u as x}from"./breadcrumb-CzFWtd6d.js";const g={"admin-reseau":{initials:"JD",prenom:"Jean",nom:"Dupont",email:"jean.dupont@ferme-du-bocage.fr",tel:"+33 6 12 34 56 78",extraPhones:[{nom:"Sophie Dupont",numero:"+33 6 98 76 54 32"}],apiKeys:[{id:1,name:"Application mobile",created:"2026-01-10"},{id:2,name:"Intégration Agreo",created:"2026-02-28"},{id:3,name:"Script export nuit",created:"2026-04-01"}]},"adherent-reseau":{initials:"MM",prenom:"Marie",nom:"Martin",email:"marie.martin@ferme-du-bocage.fr",tel:"+33 7 52 18 93 46",extraPhones:[],apiKeys:[{id:1,name:"Application mobile",created:"2026-02-14"}]}};function f(){const e=localStorage.getItem("menuRole")||"admin-reseau";return g[e]||g["admin-reseau"]}document.addEventListener("DOMContentLoaded",()=>{x(),v()});function l(e){const t=document.createElement("div");t.className="toast",t.textContent=e,document.body.appendChild(t),setTimeout(()=>t.classList.add("show"),10),setTimeout(()=>{t.classList.remove("show"),setTimeout(()=>t.remove(),300)},2500)}function v(){const e=document.getElementById("content");if(!e)return;const t=f(),s=localStorage.getItem("avatarUrl"),n=s?`style="background-image:url(${s});background-size:cover;"`:"";e.innerHTML=`
    <div class="account-page">

      <!-- Avatar + nom -->
      <section class="account-section account-section--avatar">
        <div class="account-avatar" id="account-avatar" ${n}>${s?"":t.initials}</div>
      ...
      <section class="account-section">
        ...
        <div id="extra-phones-list">
          ${(t.extraPhones||[]).map((a,i)=>`
          <div class="account-field-row extra-phone-row" data-index="${i}">
            <input type="text" class="account-input" style="max-width:160px" placeholder="Nom" value="${a.nom}" data-extra-phone-nom="${i}">
            <input type="tel" class="account-input" placeholder="Numéro" value="${a.numero}" data-extra-phone-num="${i}">
  ...
  `,A()}function k(){const e=[{key:"orgs",label:"Organisations",icon:"bi-building"},{key:"capteurs",label:"Capteurs",icon:"bi-broadcast"},{key:"alertes",label:"Alertes",icon:"bi-bell"}],t=[{key:"email",label:"Email",icon:"bi-envelope"},{key:"sms",label:"SMS",icon:"bi-phone"},{key:"push",label:"Notification push",icon:"bi-bell"}],s=[{value:"reseau",label:"Mon réseau"},{value:"organisation",label:"Mon organisation"},{value:"secteur",label:"Mon secteur"}],n={orgs:"organisation",capteurs:"reseau",alertes:"secteur"};return`<div class="notif-prefs-grid" style="grid-template-columns: 1fr repeat(3, auto) auto">
    <div></div>
      ...
      ${t.map(i=>`
        <div class="notif-prefs-cell">
          <label class="toggle-switch">
            <input type="checkbox" ${a.key==="capteurs"&&i.key==="sms"?"":"checked"}>
            <span class="toggle-track"></span>
          </label>
        </div>
      `).join("")}
      <div class="notif-prefs-cell">
        <select class="notif-peri-sel" data-cat="${a.key}">
          ${s.map(i=>`<option value="${i.value}"${i.value===n[a.key]?" selected":""}>${i.label}</option>`).join("")}
        </select>
      </div>
    `).join("")}
  </div>`}const b=20;function y(){const e="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";return"sk-wnt-"+Array.from({length:32},()=>e[Math.floor(Math.random()*e.length)]).join("")}let c=f().apiKeys.map(e=>({...e,key:y()})),E=c.length+1;function u(){const e=document.getElementById("api-keys-list"),t=document.getElementById("api-key-count"),s=document.getElementById("add-api-key-btn");e&&(t&&(t.textContent=`(${c.length} / ${b})`),s&&(s.disabled=c.length>=b),e.innerHTML=c.map(n=>`
  ...
  `).join("")||`<div style="color:var(--txt3);font-size:13px;padding:8px 0">Aucune clé d'API.</div>`,e.querySelectorAll(".api-key-copy").forEach(n=>{n.addEventListener("click",()=>{var a;(a=navigator.clipboard)==null||a.writeText(n.dataset.key),l("Clé copiée dans le presse-papier.")})}),e.querySelectorAll(".api-key-regen").forEach(n=>{n.addEventListener("click",()=>{const a=c.find(i=>i.id===parseInt(n.dataset.id));!a||!confirm(`Régénérer la clé "${a.name}" ? L'ancienne sera invalidée.`)||(a.key=y(),u(),l("Clé régénérée."))})}),e.querySelectorAll(".api-key-delete").forEach(n=>{n.addEventListener("click",()=>{const a=c.find(i=>i.id===parseInt(n.dataset.id));!a||!confirm(`Supprimer la clé "${a.name}" ?`)||(c=c.filter(i=>i.id!==a.id),u(),l("Clé supprimée."))})}),e.querySelectorAll(".api-key-rename-btn").forEach(n=>{n.addEventListener("click",()=>{const a=c.find(d=>d.id===parseInt(n.dataset.id));if(!a)return;const i=e.querySelector(`.api-key-name[data-id="${a.id}"]`);if(!i)return;const o=document.createElement("input");o.type="text",o.className="account-input",o.value=a.name,o.style.cssText="width:180px;padding:3px 8px;font-size:13px",i.replaceWith(o),o.focus();const r=()=>{a.name=o.value.trim()||a.name,u()};o.addEventListener("blur",r),o.addEventListener("keydown",d=>{d.key==="Enter"&&r()})})}))}function $(){var e;u(),(e=document.getElementById("add-api-key-btn"))==null||e.addEventListener("click",()=>{if(c.length>=b)return;const t=prompt("Nom de la nouvelle clé :");if(!t)return;const s=new Date().toISOString().slice(0,10);c.push({id:E++,name:t.trim()||"Nouvelle clé",key:y(),created:s}),u(),l("Clé générée.")})}const S=4;function I(){var t;const e=f();e.extraPhones||(e.extraPhones=[]),(t=document.getElementById("add-extra-phone-btn"))==null||t.addEventListener("click",()=>{e.extraPhones.length>=S||(e.extraPhones.push({nom:"",numero:""}),v())}),document.querySelectorAll(".remove-extra-phone").forEach(s=>{s.addEventListener("click",()=>{const n=parseInt(s.dataset.index);e.extraPhones.splice(n,1),v()})}),document.querySelectorAll(".extra-phone-save").forEach(s=>{s.addEventListener("click",()=>{var o,r;const n=parseInt(s.dataset.index),a=((o=document.querySelector(`[data-extra-phone-nom="${n}"]`))==null?void 0:o.value.trim())||"",i=((r=document.querySelector(`[data-extra-phone-num="${n}"]`))==null?void 0:r.value.trim())||"";e.extraPhones[n]={nom:a,numero:i},l("Numéro enregistré.")})})}let m=[{id:"A1B2-3C4D",model:"iPhone 15 Pro"},{id:"E5F6-7G8H",model:"Samsung Galaxy S24"},{id:"I9J0-K1L2",model:"Google Pixel 8"}];function h(){const e=document.getElementById("smartphones-list");if(e){if(!m.length){e.innerHTML='<div style="color:var(--txt3);font-size:13px;padding:4px 0">Aucun smartphone autorisé.</div>';return}e.innerHTML=m.map((t,s)=>`
    <div class="account-field-row" style="align-items:center">
      ...
      <button class="icon-btn remove-smartphone" data-index="${s}" title="Révoquer" style="color:var(--err)"><i class="bi bi-trash"></i></button>
    </div>`).join(""),e.querySelectorAll(".remove-smartphone").forEach(t=>{t.addEventListener("click",()=>{m.splice(parseInt(t.dataset.index),1),h(),l("Smartphone révoqué.")})})}}function L(){h()}function A(){var e,t,s,n;document.querySelectorAll(".account-save-btn").forEach(a=>{a.addEventListener("click",()=>l("Modifications enregistrées."))}),(e=document.getElementById("modify-email-btn"))==null||e.addEventListener("click",()=>l("Modification email — fonctionnalité à venir")),(t=document.getElementById("modify-pwd-btn"))==null||t.addEventListener("click",()=>l("Modification mot de passe — fonctionnalité à venir")),$(),I(),L(),(s=document.getElementById("delete-account-btn"))==null||s.addEventListener("click",()=>{confirm("Êtes-vous sûr de vouloir supprimer votre compte ? Cette action est irréversible.")&&l("Suppression de compte — fonctionnalité à venir")}),(n=document.getElementById("avatar-upload"))==null||n.addEventListener("change",a=>{var r;const i=(r=a.target.files)==null?void 0:r[0];if(!i)return;const o=new FileReader;o.onload=d=>{const p=document.getElementById("account-avatar");p&&(p.style.backgroundImage=`url(${d.target.result})`,p.style.backgroundSize="cover",p.textContent=""),localStorage.setItem("avatarUrl",d.target.result)},o.readAsDataURL(i)})}
