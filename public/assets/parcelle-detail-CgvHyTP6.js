...
Voulez-vous l'ajouter quand même ?`))return}}z({linkedSensorIds:[...x.linkedSensorIds,p]}),ht(h),Ae(),U(),ue()})}function _e(){var c;const e=document.getElementById("panel-integrations"),t=x.integrations||[],i=Xt.filter(l=>!t.includes(l));let a="";t.length===0?a+='<div class="panel-empty" style="margin-bottom:8px">Aucune intégration active</div>':a+=t.map(l=>`
    ...
    `).join(""),a+='<button id="create-alert-btn" class="action-btn" style="margin-top:6px"><i class="bi bi-plus"></i> Créer une alerte</button>',e.innerHTML=a,e.querySelectorAll(".remove-alert-btn").forEach(n=>{n.addEventListener("click",()=>{const s=parseInt(n.dataset.idx),c=[...x.alertes];c.splice(s,1),z({alertes:c}),Je()})}),document.getElementById("create-alert-btn").addEventListener("click",()=>{Gi(e)})}function Vi(){const e=[];return P.filter(i=>x.linkedSensorIds.includes(i.id)).forEach(i=>{(R[i.model]||[]).forEach(n=>{e.find(s=>s.id===n.id)||e.push({id:n.id,name:n.name,unit:n.unit})})}),Ge.forEach(i=>{e.find(a=>a.id===i.id)||e.push({id:i.id,name:i.name,unit:i.unit})}),e}function Gi(e){if(e.querySelector(".alert-create-form"))return;const i=Vi(),a=document.createElement("div");a.className="alert-create-form",a.innerHTML=`
